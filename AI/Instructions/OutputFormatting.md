...
Fork-only (Custom area) functions follow the same entry format in `docs/custom/<module>.md`, and are listed in `Custom.psd1`'s `FunctionsToExport` (the fork-owned manifest) rather than an engine module's.
