...
## [Get-DocsReferenceMarker](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Helper/Functions/Get-DocsReferenceMarker.ps1)

- **Description:** Reads the reference marker a documentation page declares about its own entries - an HTML comment that renders as nothing in docsify and on GitHub: `<!-- reference: functions windows/Custom -->`, `<!-- reference: skills -->`, `<!-- reference: guide -->` or `<!-- reference: none -->`. Returns `@{ Kind; Namespace }` for the first well-formed marker anywhere in the page, and `$null` for a missing, unrecognized or malformed one (a `functions` marker without an `<engine>/<area>` namespace, or any other kind carrying one), so a typo cannot quietly opt a page out of checking. Used by `List-Functions` and the Infrastructure function-reference test, which is the point: both must agree exactly on which sovereign pages are a function reference.
- **Parameters:** -Path
- **Usage:** `Get-DocsReferenceMarker -Path "C:\Repo\docs\custom\application.md"`, `(Get-DocsReferenceMarker -Path "C:\Repo\docs\custom\ai.md").Kind`

The `docs/custom/` area is sovereign: a fork puts anything there that is not ready to go upstream, so it holds functions, hand-written Agent Skills and prose side by side. The man-style `## [Name](url)` heading is a documentation convention, not a declaration of type, and treating the shape itself as "this is an exported function" made a skills page silently claim a contract it could not keep. Declaring the contract in the page fixes that and puts ownership where it travels: a page carries its own terms when it moves between repositories, and an engine claims only the namespaces it owns, so a fork tracking more than one upstream never has two engines' rules fighting over the same directory. See the [Custom area docs](../custom/README.md) for the page-author's view.

**See also:** [List-Functions](#list-functions)
...
## [Invoke-RerunLastCommandExit](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Helper/Functions/Invoke-RerunLastCommandExit.ps1)

- **Description:** Executes the scriptable exit seam used by `Rerun-LastCommand` once the fresh shell has been opened and the original window has been closed. Invokes the configured script-scoped exit action when a test seam is present, otherwise exits the current process cleanly with code `0`.
- **Usage:** `Invoke-RerunLastCommandExit`

This helper centralizes the process-exit step of the rerun path so it can be overridden in tests. When `$script:RerunLastCommandExitAction` is set it runs that action; otherwise it calls `[Environment]::Exit(0)`. `Exit` skips every `finally` block in the process, so `Rerun-LastCommand` releases stuck keyboard modifiers via `Reset-KeyboardModifiers` immediately before calling this - stuck-modifier state is OS-global and would otherwise survive the process. Keeping the exit behind this seam is what lets the close-and-respawn tail of the rerun be exercised without terminating the calling test session. `Invoke-TerminateWindowsTerminalTabsExit` is the same arrangement for `Terminate-WindowsTerminalTabs`.

**See also:** [Rerun-LastCommand](helper.md#rerun-lastcommand), [Invoke-TerminateWindowsTerminalTabsExit](system.md#invoke-terminatewindowsterminaltabsexit), [Reset-KeyboardModifiers](window.md#reset-keyboardmodifiers)
...
Reads the man-style `## [FunctionName](url)` entries from the docs pages (located via `MachineSpecificPaths.Projects.Self.Docs`) and parses each entry's contiguous `- **Key:** value` bullet block into module categories; `docs/custom/` pages merge into their mirror module's category, so fork-only functions are listed and checked alongside engine ones (the Custom area's `README.md` landing page is skipped). Only the sovereign pages that declare themselves a function reference are read: the area also holds Agent Skills and prose guides, whose entries use the same heading shape, so each page states its contract in a marker this function reads with [Get-DocsReferenceMarker](#get-docsreferencemarker) and only `<!-- reference: functions windows/Custom -->` pages are parsed. Without that a skill entry was harvested as a function and reported as an undocumented one. The default run prints every category with its functions and a total count. The discrepancy check compares the documented functions against functions actually loaded from the modules under `MachineSpecificPaths.Projects.Self.Modules`, honoring `Configuration.FunctionDiscrepancyExclusions` in both directions, and flags any that are documented but not loaded (or loaded but not documented). A name on the exclusion list is skipped either way, which is what lets a documented standalone script and a profile-defined function both stay legal. A documented function counts as loaded when it resolves at all, not only when its own module reports it - the profile redefines a few functions in global scope, which drops the module attribution the per-module enumeration relies on. Output colors come from `Configuration.ListFunctionsColors`. The parameter sets are mutually exclusive, so only one filter may be used at a time.
...
- **Description:** Reruns a recent command in a fresh, non-admin PowerShell shell. When an operation fails in a way that requires a clean shell environment (e.g., RPC errors, layout verification failures), it reads the PSReadLine history, offers the recent non-rerun commands for selection (or auto-selects the most recent with `-AutoAccept`; an exact caller-supplied `-Command` bypasses the history entirely), opens Windows Terminal in the current directory running the selected command, and closes the original window by posting `WM_CLOSE` to its handle - a deterministic close that needs no focus and synthesizes no input. Caller-specific recovery such as workspace layout, FancyZones, desktop, cache, or RPC preparation is handled before invoking this generic rerun helper. It releases stuck keyboard modifiers and a stranded mouse button via `Reset-KeyboardModifiers` (Window module, when available) before the tab-close passes, and again as its last act before exiting through `Invoke-RerunLastCommandExit`, healing the stuck-modifier input lockup.
...
Recovers from failures that require a pristine shell by reading the PSReadLine history file (`HistorySavePath`) and presenting the last N typed commands, filtered to skip blanks, duplicates, and prior `Rerun-LastCommand` invocations. When `-Command` is supplied, that exact command is rerun and PSReadLine history is not consulted at all - the shared history file is written incrementally by every open pwsh session, so its most recent line can be a command typed in another window; `Open-Workspace` records its resolved invocation through `Set-WorkspaceRerunCommand` (Window module state, never the environment) and `Set-WorkspaceWindowLayout` passes it through on escalation. The chosen command is run via `Open-Terminal` in a new Windows Terminal tab, prefixed with a `Set-Location` back to the original working directory. The `AppActivate` call that focuses Windows Terminal before the respawn loads the `Microsoft.VisualBasic` assembly first inside a try/catch, so a missing assembly can no longer abort the respawn after the retry markers were persisted. After launching, the original window is closed by posting `WM_CLOSE` straight to its handle and the process exits through `Invoke-RerunLastCommandExit`.

The close used to be a synthesized `Ctrl+Shift+W` sent to the refocused window - a shortcut that closes the very window hosting this process, so Windows Terminal tore the process down mid-injection, before the Ctrl and Shift key-ups were sent. Both modifiers then stayed logically held for the rest of the desktop session (the fresh shell typed uppercase and PSReadLine read Enter as `Shift+Enter`, so commands stopped submitting), and no self-heal could catch it because nothing runs after a process closes its own host window. `WM_CLOSE` needs no focus and synthesizes no input, so it closes the window whether or not it is foreground. One behavioral note: if `Terminate-WindowsTerminalTabs` failed to close a tab, closing the window may raise Windows Terminal's own close-all-tabs confirmation, which its `confirmCloseAllTabs` setting governs.
