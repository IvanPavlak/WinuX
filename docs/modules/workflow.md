...
- **Description:** Filters one workspace's configured `WorkspaceActions` entries down to the ones that apply on this machine, honouring two optional per-action scope keys. `Machine = "PC/Work"` is matched against the detected machine type (`$global:MachineType`), exactly like the `TaskbarConfiguration` rows and the app CSVs' `Machine` column - for identity-shaped actions such as a Work-only `Open-Outlook`. `LayoutMachine = "Laptop/Work"` is matched against the layout set [Get-LayoutMachineType](window.md#get-layoutmachinetype) resolves (a non-empty `LayoutMachineTypeOverrides` entry, else `SmallDisplayMachineType` on a small display, else the detected type) - the set `Set-WorkspaceWindowLayout` reads the layout file from - for display-shaped actions such as `Open-Browser -Instances 2`, whose window count has to agree with the layout that will be applied even while a machine is redirected to another set. Both keys take the scope string [Test-MachineTypeScope](bootstrap.md#test-machinetypescope) understands (`All`, one type, or several separated by `/`; an array of tokens works too); an absent or blank key means every machine, and an action runs only when both scopes match. Entries come back unchanged and in configured order. Called by [Open-Workspace](#open-workspace) before anything runs and by [Measure-WorkspaceOpen](#measure-workspaceopen) before its pre-checks.
...
**Tokens are validated, never guessed.** `Machine` tokens must exist in `ValidMachineTypes`. `LayoutMachine` tokens may additionally be any non-empty `LayoutMachineTypeOverrides` value or the `SmallDisplayMachineType`, because a layout set such as `Temp` is not a machine type and has no `ValidMachineTypes` entry (`Test-MachineTypeScope -AdditionalValidTypes`). A token that is neither is reported through `Write-LogError` with the workspace and action named (`WorkspaceActions.Trading [Open-Browser]`) and contributes nothing to the match, so a typo can never silently skip or keep an action. A blank scope string is treated as `All` rather than reported - the key is optional.

**Lazy and cheap.** `Get-LayoutMachineType` is called at most once per resolution, and only when some entry actually carries `LayoutMachine` and `-LayoutMachineType` was not supplied; a workspace without the key costs nothing beyond the machine-type lookup. Without the Window module the layout set falls back to the detected machine type. When a non-empty list resolves to nothing, one warning names the machine type (and, when it was consulted, the layout set) that excluded every action; `Open-Workspace` then skips the workspace like an unconfigured one instead of recording an open that produced nothing, and `Measure-WorkspaceOpen` refuses to measure it.
...
| `-MachineType`       | string   | `$global:MachineType`   | The detected machine type the `Machine` scopes are matched against.                                                                              |
| `-LayoutMachineType` | string   | resolved on demand      | The layout set the `LayoutMachine` scopes are matched against. Omit it to have `Get-LayoutMachineType` resolve it when an entry needs it.       |
...
```powershell
# One workspace, one Google window on Laptop/Work, two on the PC's own layout set
$actions = @(
...
# On a machine whose layout set is Work: three entries - the Instances = 2 opener is skipped

# Resolve for an explicit machine type and layout set without touching the live machine
Resolve-WorkspaceActions -Actions $actions -Workspace "LeagueOfLegends" -MachineType "Laptop" -LayoutMachineType "Laptop"
```
