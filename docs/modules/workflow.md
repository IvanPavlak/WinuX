...
- **Windows** are matched by [Resolve-TrackedWorkspaceWindow](#resolve-trackedworkspacewindow): recorded handle, then re-resolved by process id + process name only when that process has a single live window (Electron applications recreate their window without restarting), then by process name + **exact** title (the application restarted outright). A record that named no process is never matched on title alone - a title is not evidence of ownership.
- **A window matched by its own live handle is unambiguously this workspace's.** The survivor guard - the handles and process/title identities of every workspace that stays open, claimed up front - applies in full to a *re-resolved* window, but only its handles apply to an exact handle match. Process name plus title is **not** unique across workspaces: open WinuX and Client and each has a `YouTube - Mozilla Firefox` and a `New chat - Claude - Mozilla Firefox`. Guarding an exact match on identity therefore left the closing workspace's own YouTube and AI windows on screen, protected by the other workspace's identically titled ones.
...
- **Description:** Resolves what a plain workspace open must leave alone: tracked `-Alongside` workspaces that still have at least one live window. Returns their tracker entries verbatim plus a `HashSet[IntPtr]` of the live window handles they own, or `$null` when there is nothing to preserve. This is what makes a plain rerun of workspace A stop destroying a workspace B that is open alongside it. The alongside instance of a workspace named in `-Opening` is never preserved: a plain open of that workspace adopts and re-lays it out instead of launching a second copy beside it.
- **Parameters:** -Opening, -StatePath
- **Usage:** `Get-WorkspaceOpenProtection -Opening 'Server'`, `Get-WorkspaceOpenProtection -StatePath $trackerPath`
...
The protection set is derived the same way [Close-Workspace](#close-workspace) derives what a teardown must spare: from the tracker [Save-WorkspaceState](#save-workspacestate) wrote, never from configuration. Every tracked entry with `Alongside = $true` is checked against the live windows; an entry with at least one live window is **preserved** - its tracker entry travels forward verbatim (dead records included, the same staleness `Close-Workspace` tolerates) and its resolved handles become untouchable for the whole open. Records are resolved with `Close-Workspace`'s ladder, [Resolve-TrackedWorkspaceWindow](#resolve-trackedworkspacewindow): live handle first, then same `ProcessId` + `ProcessName` when that process has exactly one live window (Electron applications recreate their window without restarting), then same `ProcessName` + exact `Title` (the application restarted outright) - the third step sharing that function's accepted false-positive risk, since two workspaces routinely hold identically titled windows. Windows Terminal records resolve by handle only: all terminal windows share one process and a generic title, so a dead alongside terminal record used to re-resolve onto the plain session's own terminal, protect it, and leave the plain layout reporting it as "Window not found". Plain entries are never preserved: a plain rerun replaces the plain session by design. Nor is the alongside entry of a workspace this open is itself opening (`-Opening`, matched case-insensitively): `Open-Workspace Server` after `Open-Workspace Server -Alongside` is a re-layout of the same workspace, and protecting its own instance would hide its windows from every "already open" check, launch duplicates of all of them, and carry forward a tracker entry describing windows the plain session now owns. The common case - no alongside workspace tracked - pays one file parse and short-circuits to `$null` before any window enumeration.
...
| Parameter    | Description                                                                                  |
| ------------ | ---------------------------------------------------------------------------------------------- |
| `-Opening`   | Names of the workspaces the plain open is opening. Their own alongside entries are adopted, not preserved. |
| `-StatePath` | Full path to the tracker file. Defaults to [Get-WorkspaceStatePath](#get-workspacestatepath). Mainly a test seam. |
...
- **Usage:** `Measure-WorkspaceOpen`, `Measure-WorkspaceOpen Client Asseto`, `Measure-WorkspaceOpen WinuX -Setting FancyZonesApplyMethod -Runs 8`, `Measure-WorkspaceOpen WinuX -FullFactorial -Runs 3 -MaxMinutes 30`, `Measure-WorkspaceOpen WinuX -DryRun`
...
| `-Variant`       | Explicit variants: one hashtable each, configuration key to value, optional `Name`.                                                          |
| `-WarmUp`        | Opens run first under the baseline and excluded from every summary. `1` by default.                                                          |
| `-SettleSeconds` | Pause between teardown and the next open. `5` by default.                                                                                    |
...
Reads project definitions from `ProjectTerminals` in `Configuration.psd1`. Each project lists one or more `Paths`; every path opens in its own tab named `ProjectName.PathKey`. Regular path keys resolve through `PathTemplates` via `Resolve-ProjectPath`, while special entries cover shells and explicit locations. When `InvokeOnefetch` is on, each resolved-path tab runs [`Invoke-Onefetch`](system.md#invoke-onefetch) after `Set-Location`, so the panel obeys the same `TerminalGreeting.Onefetch` settings - `Enabled`, `Arguments`, and the silent skip for a path that is not a repository - as it does on `c`. The terminal greeting cannot cover those tabs and no configuration can make it: a tab is spawned as `pwsh -NoExit -EncodedCommand <Set-Location ...>`, and PowerShell runs the profile BEFORE the encoded command, so the greeting tests whatever directory Windows Terminal started the tab in rather than the project it is about to move to. Appending the call after `Set-Location` is the only point at which the tab is standing in the repository. The default comes from `TerminalGreeting.Onefetch.InProjectTerminals`, for people who want the panel on `c` but not in every project tab; passing `-InvokeOnefetch` explicitly wins for that call. Consecutive `pwsh` tabs of one project are queued and flushed as one batched `Open-Terminal` call - a single ordered `wt` invocation - while WSL tabs (which use a different Windows Terminal profile) flush the queue first so the on-screen tab order matches the configured order; a WSL tab written as `@{ Key = "WSL"; Path = "<path>" }` starts in the project instead of the distribution's home, by running `wsl.exe -d <distro> --cd <path>` as the tab's commandline in place of the profile's own. `wt -d` cannot do it: `-d` sets the Win32 working directory of the profile process, so a WSL path is refused outright and a Windows path would still lose to the profile's `--cd ~`. A commandline given to `new-tab` overrides the profile's commandline while every other profile setting still applies, and `wsl --cd` takes the path as WSL sees it - which is why it is written that way and passed through untranslated; without a shared project window, each tab still gets its own window GUID. Named-tab focusing navigates with `Ctrl+Shift+Tab` relative to the last created tab, so it applies only when tabs share a window.

| Parameter         | Default  | Description                                                                                    |
| ----------------- | -------- | ---------------------------------------------------------------------------------------------- |
| `-Project`        | -        | One or more project names from `ProjectTerminals`. Omit to show an interactive selection menu. |
| `-InvokeOnefetch` | `TerminalGreeting.Onefetch.InProjectTerminals` (ships `$true`) | Appends `Invoke-Onefetch` to each path tab's command, after the `Set-Location`, so the tab shows the repository panel. Passing it explicitly wins for that call. |
...
| `"WSL"`                | `"WSL"`                               | WSL tab using the configured `DefaultWSLDistribution`, at the distribution's home directory |
| WSL hashtable with Path | `@{ Key = "WSL"; Path = "/mnt/c/Users/Me/Repo" }` | The same WSL tab, started in that directory. Write the path as WSL sees it - `/mnt/c/...` for a Windows-mounted project, `/home/...` for a native clone; it is passed through untranslated |
...
Reads the workspace list from `Configuration.Workspaces` and the per-workspace action sequence from `Configuration.WorkspaceActions`. With no argument it shows an interactive menu (multiple selections allowed); pressing Enter there opens the workspace named by `Configuration.DefaultWorkspace` (`Default` out of the box), which the prompt names explicitly. The default is only offered when it has a `WorkspaceActions` entry - set `DefaultWorkspace = ""` (or point it at a workspace with no actions) and the prompt reads `press [Enter] to cancel` instead, with Enter exiting without opening anything. The fallback covers the interactive Enter only: a mistyped `-Workspace` argument resolves to nothing and exits rather than silently opening the default. A typical workspace opens browser tab groups, the Obsidian vault, the project (Visual Studio, VS Code, terminals), DBeaver, communication apps, and finally applies the window layout. Actions can receive the workspace's project context through the **`{SelectedProjects}` token**: a configured parameter whose FULL value is the literal string `"{SelectedProjects}"` resolves at runtime to the explicit `-Project` argument, otherwise to the projects returned by this workspace's `Open-Project` action; when neither exists the parameter is dropped so the consuming action can no-op or apply its own default. Declare token consumers - [Open-ProjectSwagger](#open-projectswagger) being the one that ships - AFTER the `Open-Project` action. Every action that declares a **`CurrentWorkspace`** parameter receives the name of the workspace being opened (`Get-FilteredParams` drops it from the rest; a configured `Parameters` value of that name wins) - [Open-Obsidian](application.md#open-obsidian) uses it to load the same-named Obsidian workspace. An action that declares a **`Deferred`** parameter receives it the same way, and hands its slow tail back to the flow: it launches its application, queues the remainder through [Register-DeferredAction](helper.md#register-deferredaction) and returns at once. [Open-Obsidian](application.md#open-obsidian) is the one that ships - its CLI cannot answer until Obsidian has finished starting, so every opener queued behind it used to wait out that startup - and every queued tail is run by [Complete-DeferredActions](helper.md#complete-deferredactions) immediately before the `Set-WorkspaceWindowLayout` action, or when the action list ends without one. Any opener with the same shape of wait can join by declaring the switch and registering its tail; nothing in the flow names Obsidian. That point is deliberate rather than convenient: the layout holds a window stable only while its title stops changing, and loading an Obsidian workspace retitles its window. Any extra `-Name Value` / `-Switch` arguments are forwarded to the underlying actions, with the workspace configuration taking precedence. Passing `-VSCodeWorkspace <name>` (or a bare `-VSCodeWorkspace` for a selection menu, or a `DefaultVSCodeWorkspaces` config entry for the workspace) opens `<name>.code-workspace` in place of the project folder - the resolved name (precedence: command-line value, then the bare-switch menu, then the config default) is passed to the `Open-Project` action, which reroutes `Open-VSCode` to `Open-VSCodeWorkspace`. The window layout needs no coupling to this: VS Code layout entries match by process, so the workspace window lands in the VS Code slot like any other VS Code window.

**Actions can be scoped to machines.** An entry may carry `Machine = "PC/Work"` (runs only on those detected machine types) and/or `LayoutMachine = "Laptop/Work"` (runs only when the layout set [Get-LayoutMachineType](window.md#get-layoutmachinetype) resolves is one of those - the set the layout action reads its file from, so a count-shaped opener such as `Open-Browser -Instances 2` can follow the layout even while the machine is redirected to another set through `LayoutMachineTypeOverrides`). Absent or blank keys mean every machine. An action that runs everywhere but differs per machine keeps one entry and carries the differences in `MachineParameters` / `LayoutMachineParameters` tables (`@{ "<scope>" = @{ ... } }`) merged over its `Parameters`, a `$null` removing a parameter. The list is passed through [Resolve-WorkspaceActions](#resolve-workspaceactions) once per workspace, before the early layout preparation and the action loop, so both see the same actions and the same merged parameters; a workspace whose every action is scoped to another machine is reported and skipped exactly like one with no actions - nothing runs and nothing is recorded, so `Close-Workspace` is never handed an open that produced nothing.
...
**The layout preamble runs before the launch actions.** The RPC probe, the layout file and its validation, the virtual desktop resize and the FancyZones zone layouts depend on no window existing, yet they ran inside the layout action - last, once every application had been launched and was competing for the CPU: 3.4 s on a cold 8-desktop open against 0.2 s on an idle machine, the probe runspace alone 1.7 s. When the workspace has a `Set-WorkspaceWindowLayout` action and `WorkspaceLayoutPrepareEarly` is not `$false`, the open now calls `Set-WorkspaceWindowLayout -PrepareOnly` before the first action, with the action's own parameters plus the same alongside offset, protected handles and pre-open capture the action loop threads in, so the prepared state is exactly what the action would have produced. The layout action then finds the desktop count right and the zone layouts in the file and skips both. A window-only retry is never prepared (the marker's process copy is read without consuming the mirror; the retry's point is to leave the desktops alone), and a failed preparation only warns and leaves the work to the action. The preparation is timed as `Set-WorkspaceWindowLayout -PrepareOnly` in the benchmark row's `Actions` and counted as layout time, and its phase record is folded into the layout action's record, so `Get-WorkspaceBenchmark` shows the `Desktops` and `FancyZones` seconds wherever they were spent.

Once the workspace names are resolved (including interactive menu picks), the exact invocation - resolved workspace names, `-Project`, `-Alongside`, and any extra arguments - is recorded in-process through [Set-WorkspaceRerunCommand](window.md#set-workspacereruncommand) (cleared in the `finally` block and before a terminating action exits), so a failure-path respawn via `Set-WorkspaceWindowLayout`'s escalation to `ReRun-LastCommand` reruns precisely this command instead of scraping the shared PSReadLine history. Neither that record nor the elapsed-time base is a process environment variable any more: both used to be, and every application and terminal tab the open spawned inherited them, so a later `Open-Workspace` typed into such a tab reported the time since the earlier open (597 s and 704 s showed up in the session logs) and a standalone layout escalation there would have respawned the inherited open. The one legitimate hand-over remains: the alongside bootstrap command sets `OPEN_WORKSPACE_START_UTC` for the relaunched shell so its summary includes the relaunch, and that shell consumes it at entry, before anything it spawns could inherit it. In alongside mode the desktop offset is [Get-VirtualDesktopCount](window.md#get-virtualdesktopcount) - desktops are 0-based, so the count is the first index that does not exist yet; the read goes through the Window module's seam, which reconnects a stale COM session before answering. When it throws (virtual desktop enumeration failed even after recovery), that workspace is skipped with a clear error instead of opening on top of the current one. And because a configured `Terminate-WindowsTerminalTabs` action with `-OnlyCurrent` or `-IncludeCurrent` ends the process via `[Environment]::Exit` - which skips `finally` blocks - the elapsed summary is printed and stuck keyboard modifiers are released before control passes to that action.
...
## [Read-WorkspaceBenchmark](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Workflow/Functions/Read-WorkspaceBenchmark.ps1)

- **Description:** The read side of the benchmark file, shared by [Get-WorkspaceBenchmark](#get-workspacebenchmark) and [Measure-WorkspaceOpen](#measure-workspaceopen) so the two can never disagree about a row. Reads `WorkspaceBenchmark.csv` ([Get-WorkspaceBenchmarkPath](#get-workspacebenchmarkpath), or `-BenchmarkPath`) and returns every row as an object with typed numbers - the seconds columns as doubles, `Attempts` as an int, parsed culture-invariant exactly as [Write-WorkspaceBenchmark](#write-workspacebenchmark) wrote them, an unparseable cell reading as `0` - sorted by `Timestamp`, with rows written within the same second kept in file order. A file that does not exist yet reads as an empty result; a file that cannot be read throws, so the caller decides how to report it. No filtering, no title, no warning: the callers add those.
- **Parameters:** -BenchmarkPath
- **Usage:** `Read-WorkspaceBenchmark | Where-Object Workspace -eq MyWorkspace | Select-Object -Last 1`, `@(Read-WorkspaceBenchmark).Count`
...
## [Resolve-TrackedWorkspaceWindow](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Workflow/Functions/Resolve-TrackedWorkspaceWindow.ps1)

- **Description:** Finds the live window a workspace tracker record refers to, returning `@{ Window; Exact }` (`Exact` is `$true` for a live-handle match) or `$null`. The ladder: exact handle; then same `ProcessId` + `ProcessName` only when that process has exactly one live window (the Electron recreate case - a multi-window process gives no way to tell which window is the recorded one); then same `ProcessName` + exact `Title`. Windows Terminal records resolve by handle only, and a record naming no process is never re-resolved. Shared by [Close-Workspace](#close-workspace) and [Get-WorkspaceOpenProtection](#get-workspaceopenprotection).
- **Parameters:** -Record, -LiveWindows
- **Usage:** `Resolve-TrackedWorkspaceWindow -Record $record -LiveWindows (Get-WindowHandle)`

## [Resolve-WorkspaceActions](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Workflow/Functions/Resolve-WorkspaceActions.ps1)

- **Description:** Resolves one workspace's configured `WorkspaceActions` entries for this machine - which ones run, and with which parameters - through four optional per-action keys. Two **scope** keys decide whether an action runs: `Machine = "PC/Work"` is matched against the detected machine type (`$global:MachineType`), exactly like the `TaskbarConfiguration` rows and the app CSVs' `Machine` column - for identity-shaped actions such as a Work-only `Open-Outlook`; `LayoutMachine = "Laptop/Work"` is matched against the layout set [Get-LayoutMachineType](window.md#get-layoutmachinetype) resolves (a non-empty `LayoutMachineTypeOverrides` entry, else `SmallDisplayMachineType` on a small display, else the detected type) - the set `Set-WorkspaceWindowLayout` reads the layout file from. Two **table** keys vary an action that runs everywhere: `MachineParameters = @{ "<scope>" = @{ ... } }` and `LayoutMachineParameters = @{ "<scope>" = @{ ... } }` hold, per machine scope, the parameters that differ, and every row covering this machine is deep-merged over a copy of `Parameters` - so `@{ Action = "Open-Browser"; Parameters = @{ Groups = @("Google") }; LayoutMachineParameters = @{ PC = @{ Instances = 2 } } }` opens two windows on the PC's own layout set and one everywhere else, including on the PC while it is redirected to another set. Scopes and row keys take the scope string [Test-MachineTypeScope](bootstrap.md#test-machinetypescope) understands (`All`, one type, or several separated by `/`; an array of tokens works for the scope keys); an absent or blank scope means every machine, an action runs only when both scopes match, and a parameter a row sets to `$null` is removed. Entries come back in configured order, unchanged unless a table applied. Called by [Open-Workspace](#open-workspace) before anything runs and by [Measure-WorkspaceOpen](#measure-workspaceopen) before its pre-checks.
- **Parameters:** -Actions, -Workspace, -MachineType, -LayoutMachineType, -Configuration
- **Usage:** `Resolve-WorkspaceActions -Actions $Configuration.WorkspaceActions['LeagueOfLegends'] -Workspace 'LeagueOfLegends'`, `Resolve-WorkspaceActions -Actions $actions -Workspace 'Server' -MachineType 'Laptop' -LayoutMachineType 'Laptop'`

**Why two keys.** Most machine-typed things in WinuX describe the machine's identity, and `Machine` covers those. Window arrangement describes its **monitors**, which is why the layouts can be redirected to another set without changing the machine type ([Layout Set Overrides](../configuration/configuration-reference.md#layout-set-overrides)). An action that exists to produce windows for the layout - a browser opened twice so two zones can be filled - has to follow that redirect too, or the desktop running on a single screen opens two windows for a layout that wants one. `LayoutMachine` follows it; `Machine` does not. The `Machine` scope of a PC redirected to the Work layouts is still `PC`, so a Work-only `Open-Outlook` stays closed there while a `LayoutMachine = "Work"` browser opener runs.

**Varying an action instead of duplicating it.** The scope keys include or exclude a whole entry, which for an action that differs only in one parameter means writing it twice. The tables are the `MachineOverrides` idea applied to one action: `Parameters` is the default, a row is the difference for the machines its key covers. Rows are matched like scopes (`MachineParameters` against the detected type, `LayoutMachineParameters` against the layout set, with the same token rules), and every matching row is deep-merged over a **copy** of `Parameters` - nested hashtables merge key by key, everything else is replaced - in a fixed order: the `All` row first (the table's default row), then the other matching rows alphabetically, `MachineParameters` before `LayoutMachineParameters`, so a display-shaped value wins over an identity-shaped one (the layout set is the more specific statement about the machine as it is running now, and a window count has to agree with the layout file that will actually be read). A parameter a row sets to `$null` is removed, which is how a machine drops a parameter as well as changes one; an explicit `$null` would otherwise be bound by the action, and a present key would keep `Open-Workspace` from filling in a command-line value. Only top-level parameters are removed that way. The configured entry is never modified: an entry with a table comes back as a new hashtable carrying the merged `Parameters` and every other key except the tables, an entry without tables comes back as the same object. A table that is not a hashtable, or a row that is not one, is reported with the workspace, action and table named and ignored. Tables were chosen over per-parameter maps (`Instances = @{ PC = 2; Default = 1 }`) because a parameter's value can legitimately be a hashtable, so a map would need a marker to be told apart, and because one row per machine keeps everything that differs on one machine in one place.

**Tokens are validated, never guessed.** `Machine` tokens (and `MachineParameters` row keys) must exist in `ValidMachineTypes`. `LayoutMachine` tokens (and `LayoutMachineParameters` row keys) may additionally be any non-empty `LayoutMachineTypeOverrides` value or the `SmallDisplayMachineType`, because a layout set such as `Temp` is not a machine type and has no `ValidMachineTypes` entry (`Test-MachineTypeScope -AdditionalValidTypes`). A token that is neither is reported through `Write-LogError` with the workspace and action named (`WorkspaceActions.Trading [Open-Browser]`, `WorkspaceActions.Trading [Open-Browser].LayoutMachineParameters` for a row) and contributes nothing to the match, so a typo can never silently skip or keep an action or a row. A blank scope string is treated as `All` rather than reported - the key is optional. Because the scope check runs first, an action that does not run on this machine never reports table problems.

**Lazy and cheap.** `Get-LayoutMachineType` is called at most once per resolution, and only when some entry actually carries `LayoutMachine` or `LayoutMachineParameters` and `-LayoutMachineType` was not supplied; a workspace without those keys costs nothing beyond the machine-type lookup. Without the Window module the layout set falls back to the detected machine type. When a non-empty list resolves to nothing, one warning names the machine type (and, when it was consulted, the layout set) that excluded every action; `Open-Workspace` then skips the workspace like an unconfigured one instead of recording an open that produced nothing, and `Measure-WorkspaceOpen` refuses to measure it.

| Parameter            | Type     | Default                 | Description                                                                                                                                     |
| -------------------- | -------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `-Actions`           | object[] | -                       | The configured entries of one workspace (`$Configuration.WorkspaceActions[<name>]`); hashtables or objects. Null or empty resolves to an empty array. |
| `-Workspace`         | string   | -                       | The workspace name, used in the debug, warning and unknown-token messages.                                                                       |
| `-MachineType`       | string   | `$global:MachineType`   | The detected machine type the `Machine` scopes and `MachineParameters` rows are matched against.                                                 |
| `-LayoutMachineType` | string   | resolved on demand      | The layout set the `LayoutMachine` scopes and `LayoutMachineParameters` rows are matched against. Omit it to have `Get-LayoutMachineType` resolve it when an entry needs it. |
| `-Configuration`     | hashtable | `$global:Configuration` | Where `LayoutMachineTypeOverrides` and `SmallDisplayMachineType` are read from for the extra valid tokens; `Measure-WorkspaceOpen` hands over its own. |

```powershell
# One workspace: one Google window everywhere, two on the PC's own layout set, Outlook only at work
$actions = @(
...
Resolve-WorkspaceActions -Actions $actions -Workspace "LeagueOfLegends"
# On a PC whose layout set is Work: three entries - Outlook is skipped, the browser opener has no Instances

# Resolve for an explicit machine type and layout set without touching the live machine
Resolve-WorkspaceActions -Actions $actions -Workspace "LeagueOfLegends" -MachineType "PC" -LayoutMachineType "PC"
# The browser opener now carries Instances = 2

# Drop a parameter on some machines and change another, in one entry
@{ Action = "Open-Project"; Parameters = @{ Project = "Client"; RunApp = $true }; MachineParameters = @{ "Laptop/Work" = @{ RunApp = $null; Project = "ClientLite" } } }
```

**See also:** [Open-Workspace](#open-workspace), [Measure-WorkspaceOpen](#measure-workspaceopen), [Test-MachineTypeScope](bootstrap.md#test-machinetypescope), [Get-LayoutMachineType](window.md#get-layoutmachinetype), [Set-WorkspaceWindowLayout](window.md#set-workspacewindowlayout), [Workspace Actions](../configuration/configuration-reference.md#workspace-actions)
