...
Only then is the existing overlay copied into the unified backup sink (`Backups\Windows\Config\<name>.local\<timestamp>\`, via [`Backup-RepositoryItem`](helper.md#backup-repositoryitem)) and the candidate moved over it. The candidate is staged in the destination folder so the replace is a same-volume rename a reader can never observe half-written, and restoring the backup is a complete undo - every save keeps its own timestamped copy. A backup that cannot be taken aborts the save. `-WhatIf` runs the whole validation and reports what would be written without writing it, without backing anything up and without leaving a staged file behind.
...
| `-NoBackup`    | Do not back the previous overlay up. Not recommended; it removes the one-click undo.                                                                                                                                                                                                                     |
