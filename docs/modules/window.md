...
| `-KeepPositionedWindows`  | Appends to the positioned-window tracking instead of resetting it first (the default, so `Snap-AllWindows` sees exactly one call's windows). The per-desktop passes of one workspace open share a single tracking set and pass this on every call but the first. |
