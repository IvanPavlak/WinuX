...
Only `-MonitorConfig` is mandatory. With no `-DesktopNumber`, it covers every virtual desktop the call owns (using the VirtualDesktop module, when available). In the shortcut pass every desktop switch goes through [Switch-VirtualDesktop](#switch-virtualdesktop), which confirms the desktop is showing before the layout hotkey is injected - an unconfirmed switch skips that desktop (and the return-desktop re-apply), closing the race where a layout was silently recorded under the previous desktop's GUID - and the pass returns to the starting desktop. The function calls no VirtualDesktop cmdlet and carries no RPC retry of its own: the switch confirmation, the stale-session reconnect and the backoff all live in the Window module's seam. Layout names resolve to shortcut numbers (0-9) via `LayoutNumbers` in `Configuration.psd1`, and to layout uuids via `custom-layouts.json` for the file write. Returns a result array (one row per monitor/desktop) with a `Status` such as `Layout Written`, `Shortcut Sent`, `Already Applied`, `Monitor Not Found`, or `Failed`.
...
Builds on existing module helpers: `Get-WindowHandle` for pattern filtering when `-ProcessName`/`-WindowTitle` is supplied (otherwise `Get-CachedWindows` enumerates all windows; the cache is cleared first to read fresh positions), `Get-MonitorInfo` for monitor bounds and work areas, `Resize-Windows` (target-bounds mode, `-InsetPercent 0`) for reliable, centralized placement, `Wait-WindowRect` for post-placement verification, and `Ensure-WindowsFormsLoaded` for the `System.Windows.Forms` dependency. System and shell windows (Program Manager, Windows Input Experience, search/start surfaces, overlays, etc.) and windows with no meaningful size are skipped. Under `Set-LogLevel Verbose` it prints a per-window trace plus a diagnostics summary with enumerated, eligible, centered, and skipped counts and exclusion counts (skip-title and invalid-size) to explain why some windows were not centered.

**Each placement is verified.** `SetWindowPos` reports success for the call, not for where the window ends up, and an external window manager - FancyZones restoring a remembered zone - can move it straight back. Every placement is therefore checked with `Wait-WindowRect` (150 ms budget) and re-applied once when the window did not land on the centered bounds, the same treatment `Move-Windows` gives its monitor leg. Windows that will not hold the bounds after two attempts are reported in normal mode as a warning with their names, not only under verbose logging; previously the normal-mode summary printed only the centered count, so a pass that left windows where they were looked identical to a complete one.
...
When multiple live windows match the same `ProcessName`/`WindowTitle`, candidates are scored by how closely their actual bounds match the expected bounds and the best-matching one is selected; a `$claimedHandles` set guarantees each duplicate-keyed layout entry claims a unique window so identical titles (e.g. two browser windows) are never misassigned. A title-drift fallback handles non-browser apps whose captions change at runtime (e.g. new Outlook / `Olk` titling its window after the selected folder): when strict process-and-title matching finds nothing and exactly one unclaimed process window remains, that window is accepted rather than false-failing. The virtual desktop is part of "in place": each candidate's desktop is read through [Get-WindowDesktopIndex](#get-windowdesktopindex) (so a stale COM session is reconnected by the seam rather than answering with the wrong desktop, and an unplaceable window reads as `-1`), candidates on the expected desktop win, a candidate whose desktop resolves to another desktop is never accepted (the entry fails with `On desktop N` and the retry's `Set-WindowLayouts` moves it), and only candidates whose desktop cannot be resolved at all remain a fallback. The desktop used to be consulted only to choose between several candidates, so a lone window at the right rect on the wrong desktop passed - a VS Code window that appeared on the desktop the snap pass happened to be on, at the bounds VS Code itself restores, verified as placed. As a last resort before declaring an entry "window not found", the tracked positioned window whose expected bounds and desktop match the entry is accepted, provided its handle is still alive and unclaimed - covering browser windows whose title drifted between positioning and verification (a tab finished loading), which previously escalated to reruns that could never fix a title mismatch.
...
Unlike [Focus-VirtualDesktop](#focus-virtualdesktop) this does exactly one thing - it makes a desktop visible. No window enumeration, no focus locking, no section title, so a caller can bring a desktop up, do its work, and restore the view without narrating a "focus" step it never intended. `$null` means nothing needs restoring: either the target was already showing, or the switch could not be made. The current desktop and the window's desktop are read through [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex) and [Get-WindowDesktopIndex](#get-windowdesktopindex), and the switch itself is [Switch-VirtualDesktop](#switch-virtualdesktop), which confirms the desktop is showing, retries, falls back to a session reset when a long-running shell holds a stale VirtualDesktop COM proxy whose switch silently no-ops, and clears the window cache once the switch lands (handles enumerated beforehand describe the previous desktop's composition). This function carries no retry or recovery of its own; it only reports the outcome.

Note that `SetForegroundWindow` and the module's own `ForceForegroundWindow` are both refused by the Windows foreground lock when called from a background process, so activation is not a usable route to the same end - a virtual desktop switch is not subject to that lock, which is what makes this work. Used by [Get-TerminalTabSnapshot](helper.md#get-terminaltabsnapshot) and [Close-Workspace](workflow.md#close-workspace), both of which read or close terminal tabs on desktops the workspace layout has parked them on.
...
**See also:** [Focus-VirtualDesktop](#focus-virtualdesktop), [Switch-VirtualDesktop](#switch-virtualdesktop), [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex), [Get-TerminalTabSnapshot](helper.md#get-terminaltabsnapshot), [Close-Workspace](workflow.md#close-workspace)
...
- **Recovery:** Sits on the Window module's seam: every count, create and remove call runs through [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation) (5 attempts / 250 ms initial delay), and the first count carries `-Probe` so the live RPC endpoint is verified and repaired once, up front, before a sequence of several desktop operations. An RPC failure (a stale COM session after heavy desktop churn or an Explorer restart) reconnects the session and retries with backoff inside the seam; any other error surfaces at once. Desktop switches (after removing extras, and for `-SwitchToDesktop`) go through [Switch-VirtualDesktop](#switch-virtualdesktop), which confirms the desktop is showing. Verifies the final desktop count before returning success.
- **Parameters:** -Count, -SwitchToDesktop
- **Usage:** `Ensure-VirtualDesktops -Count 3`, `Ensure-VirtualDesktops -Count 3 -SwitchToDesktop 2`

Creates virtual desktops if fewer than `-Count` exist, up to the requested count, and removes extras (then switches to the first desktop) if more exist. After reconciling the count it can switch to a specific desktop via `-SwitchToDesktop` (1-based; `0` means don't switch). Returns `$true` on success and `$false` on failure (module missing or desktop create/remove failed). "The RPC server is unavailable. (0x800706BA)" failures - previously a known issue when Explorer had restarted earlier in the session (wallpaper, taskbar, or icon-cache operations) - now recover in place through the seam instead of failing the workspace run.
...
**See also:** [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Switch-VirtualDesktop](#switch-virtualdesktop), [Remove-VirtualDesktops](system.md#remove-virtualdesktops), [Configuration: Window Layout](../configuration/guides/window/configure-window-layout.md)
...
- **Description:** Switches to a virtual desktop and locks keyboard focus onto a window that lives there, used as the final `WorkspaceActions` step so a workspace run reliably lands the user on the first desktop. Workspace setup (`Set-WorkspaceWindowLayout` / `Snap-AllWindows`) hops across every desktop while snapping windows, and the final landing has two gaps: in a long-running shell the VirtualDesktop COM/RPC session can go stale and a switch silently no-ops, and even when it takes, nothing guarantees a foreground window on the target desktop - so focus can revert to a window snapped on a higher desktop and drag the view back with it. This function closes both by switching through [Switch-VirtualDesktop](#switch-virtualdesktop) (which confirms the desktop is showing, retries and falls back to a session reset) and then parking focus on a real window on the target desktop.
- **Parameters:** -DesktopNumber, -DesktopOffset
- **Usage:** `Focus-VirtualDesktop`, `Focus-VirtualDesktop -DesktopNumber 1 -DesktopOffset 2`

Resolves the target desktop via `ConvertTo-InternalDesktopIndex` (combining `DesktopNumber` and `DesktopOffset`), switches with `Switch-VirtualDesktop` - the same call `Snap-AllWindows` and `Ensure-DesktopVisible` use, which confirms the switch, retries, recovers a stale COM session once and clears the window cache so the lookups below never act on the previous desktop's handles - then parks focus on a real window that lives on the target desktop, resolved through `Get-WindowDesktopIndex`. It calls no VirtualDesktop cmdlet directly and carries no retry or recovery block of its own; a switch that never lands is reported as `Failed to focus Virtual Desktop N after retries!`. The focus-target scan is terminal-first and stops at the first window resolved to the target desktop, instead of resolving every window's desktop. It prefers Windows Terminal via `Focus-TerminalTab` (only when the terminal actually lives on that desktop, since activating one elsewhere would drag the view away), handing over the **handle** of the window it verified - left to itself `Focus-TerminalTab` activates the first `WindowsTerminal` *process*, and one process hosts every one of its windows, so the check could clear one window while a sibling on another desktop is the one that actually comes forward. Otherwise it force-foregrounds the first window found there via `WindowModule.Native::ForceForegroundWindow`. Lazy-loads the VirtualDesktop module if needed and reports the outcome with a `=>` status message (focused, switched-but-no-window, or failure) rather than returning a value.
...
**See also:** [Switch-VirtualDesktop](#switch-virtualdesktop), [Ensure-DesktopVisible](#ensure-desktopvisible), [Get-WindowDesktopIndex](#get-windowdesktopindex)
...
## [Get-CurrentVirtualDesktopIndex](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Get-CurrentVirtualDesktopIndex.ps1)

- **Description:** Returns the 0-based index of the virtual desktop currently on screen. One of the two reads the Window module's seam exposes (the other is `Get-VirtualDesktopCount`): the lookup runs through `Invoke-VirtualDesktopOperation`, so a stale COM session - the state heavy desktop churn or an Explorer restart leaves behind - is reconnected and the read retried rather than reporting the desktop that was showing before the restart. Throws when the desktop manager cannot be reached at all (module missing, or RPC still dead after recovery); it never answers with a silent `$null`.
- **Usage:** `Get-CurrentVirtualDesktopIndex`, `$returnTo = Get-CurrentVirtualDesktopIndex`

Callers use it to remember where the view was before a sequence of switches and to confirm a switch landed: `Switch-VirtualDesktop` polls it until the target desktop is the one showing, `Ensure-DesktopVisible` returns it as the index to restore, and `Snap-AllWindows` reads it before hopping across the workspace's desktops. Compare the value, not its truthiness - `0` (the first desktop) is a real answer.

```powershell
# Remember the visible desktop, switch around, then put the view back
$returnTo = Get-CurrentVirtualDesktopIndex
[void](Switch-VirtualDesktop -Index 2)
[void](Switch-VirtualDesktop -Index $returnTo)
```

**See also:** [Get-VirtualDesktopCount](#get-virtualdesktopcount), [Switch-VirtualDesktop](#switch-virtualdesktop), [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation)
...
## [Get-VirtualDesktopCount](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Get-VirtualDesktopCount.ps1)

- **Description:** Returns how many virtual desktops exist, as the desktop manager reports it right now. One of the two reads the Window module's seam exposes (the other is `Get-CurrentVirtualDesktopIndex`): the count runs through `Invoke-VirtualDesktopOperation`, so a stale COM session is reconnected and the read retried instead of answering with yesterday's number - a stale count of one desktop is how every window move once reported "out of range" on a screen showing three. Because desktops are 0-based, the count is also the index the next desktop would get, which is what an `-Alongside` workspace open uses as its `DesktopOffset`. Throws when the desktop manager cannot be reached at all (module missing, or RPC still dead after recovery); it never returns `0` for "could not tell", because an alongside caller falling back to offset `0` would open the new workspace on top of the current one.
- **Usage:** `Get-VirtualDesktopCount`, `$offset = Get-VirtualDesktopCount`

Replaces the older next-free-desktop helper, which counted the desktops itself and returned `$null` on failure. [Open-Workspace](workflow.md#open-workspace) wraps the call in a `try`/`catch` and treats an exception as "abort the alongside open".

```powershell
# With 2 desktops (indices 0 and 1), returns 2 - the first index that does not exist yet
$offset = Get-VirtualDesktopCount
```

**See also:** [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex), [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Open-Workspace](workflow.md#open-workspace)
...
- **Description:** Resolves which virtual desktop a window lives on, as a 0-based index, returning `-1` for every "cannot tell" case rather than `$null` or an exception. Wraps the `Get-DesktopIndex (Get-DesktopFromWindow -Hwnd ...)` pair, run through `Invoke-VirtualDesktopOperation`, with the guards each caller of it needs: the VirtualDesktop module may be missing, the lookup throws for windows that cannot be resolved at all, and a stale COM session must be reconnected rather than reported as "no desktop".
- **Parameters:** -WindowHandle
...
The function sits on the Window module's seam. A genuine RPC failure (a stale COM session after heavy desktop churn or an Explorer restart) is classified, reconnected with `Reset-VirtualDesktopState` and retried with backoff inside `Invoke-VirtualDesktopOperation`; a lookup that fails on its own merits is rethrown by the seam at once and answered here with `-1`, because a window that cannot be resolved cannot succeed on a second attempt. A desktop manager that stays unreachable after the seam's recovery also reads as `-1`. [Remove-VirtualDesktops](system.md#remove-virtualdesktops) does its own whole-set occupancy scan through the seam for the same reason: one retry ladder per scan, never one per window. Used by [Close-Workspace](workflow.md#close-workspace) to decide which windows sit on a workspace's own desktops and which desktops to remove with it.
...
**See also:** [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Ensure-DesktopVisible](#ensure-desktopvisible), [Remove-VirtualDesktops](system.md#remove-virtualdesktops), [Close-Workspace](workflow.md#close-workspace)
...
## [Get-WorkspaceRerunCommand](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Get-WorkspaceRerunCommand.ps1)

- **Description:** Returns the exact `Open-Workspace` invocation recorded for the open in progress by [Set-WorkspaceRerunCommand](#set-workspacereruncommand), or `$null`. `Set-WorkspaceWindowLayout` calls it when its in-process retries are exhausted and the workspace has to be rerun in a fresh shell: a recorded command is handed to `ReRun-LastCommand -Command`, so the respawn reruns precisely the open that is failing and PSReadLine history is never consulted; `$null` (a standalone call, or an open that already ended) lets `ReRun-LastCommand` fall back to its history prompt.
- **Parameters:** none
- **Usage:** `Get-WorkspaceRerunCommand`

Module state rather than an environment variable, so a terminal tab spawned by the open can never inherit it - see [Set-WorkspaceRerunCommand](#set-workspacereruncommand) for why that matters.
...
- **Description:** Lazily imports the VirtualDesktop module with caching. Checks whether the module is available and imports it only once, using module-scoped state to avoid repeated `Get-Module` calls. Returns `$true` if the module is loaded and ready, `$false` otherwise. It is part of the implementation of the Window module's seam: `Invoke-VirtualDesktopOperation` and `Switch-VirtualDesktop` call it before every operation, and turn a `$false` into one clear exception carrying the install command. Functions that only need to know whether desktop work is possible at all (`Ensure-VirtualDesktops`, `Focus-VirtualDesktop`, `Ensure-DesktopVisible`, `Remove-VirtualDesktops`) call it with `-Silent` up front to fail soft instead.
- **Parameters:** -Silent
- **Usage:** `if (Import-VirtualDesktopModule) { ... }`, `$hasModule = Import-VirtualDesktopModule -Silent`

On first call it queries `Get-Module -ListAvailable` once, caches the result in module-scoped state, and short-circuits on every subsequent call. If the module is unavailable it warns with the install command (`Install-Module -Name VirtualDesktop -Scope CurrentUser`) unless `-Silent` is set; if already loaded by another source it reuses it, otherwise it imports it. Returns a boolean reflecting whether the module is loaded and ready. The function is exported because the module loader exports every file under `Functions`, but a successful import is not a working session: the compiled types cache their COM proxies per process, so after an Explorer restart the module is loaded and every cmdlet still fails with `0x800706BA`. Do not gate raw cmdlet calls on it - go through [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), which owns that failure.
...
```powershell
# Decide up front whether desktop work is possible at all (warns with the install command when it is not)
if (Import-VirtualDesktopModule) {
    $count = Get-VirtualDesktopCount
}
...
**See also:** [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Reset-VirtualDesktopState](#reset-virtualdesktopstate)
...
## [Invoke-MultiZoneWindowSnap](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Invoke-MultiZoneWindowSnap.ps1)

- **Description:** Snaps a pre-positioned window into its FancyZones zone through the attempt budget that used to live inline in `Snap-AllWindows`: the window arrives already inset **inside** its target zone (`Set-WindowLayouts` / `Resize-PositionedWindows` put it there), so FancyZones' relative `Win+Up` resolves to that zone. Each attempt acquires stable foreground focus (`Confirm-WindowForeground`), re-checks the foreground atomically right before injecting (a chord sent to the wrong window is worse than no chord), sends `Win+Up` through the batched `SendInput` helper and polls the rect with `Wait-WindowRect` until it matches the **full** zone rectangle (budget growing per attempt); when the keyboard snap does not verify it re-insets the window and runs FancyZones' real drag path (`ShiftDragSnap`), rotating the drag start point across attempts for non-browser windows (browsers always drag from the left inset so a tab is never torn off), and polls again. Retries begin by clearing the keyboard modifier state and re-insetting the window. When every attempt is exhausted it reports the window unverified and stops - what happens next (a zone-grid reset and a second round, or a recorded failure) is `Snap-AllWindows`' decision, which is what makes that recovery unit-testable.
- **Parameters:** -WindowHandle, -ExpectedX, -ExpectedY, -ExpectedWidth, -ExpectedHeight, -WindowTitle, -MaxAttempts, -InsetPercent
- **Usage:** `Invoke-MultiZoneWindowSnap -WindowHandle $handle -ExpectedX 3 -ExpectedY 3 -ExpectedWidth 1717 -ExpectedHeight 1434 -WindowTitle 'Code'`

The twin of [Invoke-SingleZoneWindowSnap](window.md#invoke-singlezonewindowsnap): same attempt shape, same result shape, but no assignment clearing and no deeper centering - on a multi-zone grid the relative move has neighbouring zones to resolve against, and the shared inset is exactly what steers it. The window's virtual desktop must be the **active** one and the window focusable.

| Parameter         | Type   | Default                  | Description                                                                                  |
| ----------------- | ------ | ------------------------ | -------------------------------------------------------------------------------------------- |
| `-WindowHandle`   | IntPtr | -                        | Handle of the window to snap. (Mandatory)                                                    |
| `-ExpectedX`      | int    | -                        | Zone left edge in physical pixels - the rect a verified snap must produce. (Mandatory)      |
| `-ExpectedY`      | int    | -                        | Zone top edge in physical pixels. (Mandatory)                                                |
| `-ExpectedWidth`  | int    | -                        | Zone width in physical pixels. (Mandatory)                                                   |
| `-ExpectedHeight` | int    | -                        | Zone height in physical pixels. (Mandatory)                                                  |
| `-WindowTitle`    | string | `''`                     | Window title used only for log messages.                                                     |
| `-MaxAttempts`    | int    | `3`                      | Keyboard-plus-drag attempts before reporting failure.                                        |
| `-InsetPercent`   | double | `Get-WindowInsetPercent` | The shared pre-snap inset the window is re-positioned at between attempts and before a drag. |

Returns a `PSCustomObject` with `Verified`, `Method` (`KeyboardSnap` \| `ShiftDrag` \| `None`), `Attempts`, the last observed `X`/`Y`/`Width`/`Height` (`$null` when no attempt got as far as reading the rect) and `Error` (the exception message when the final attempt threw).

```powershell
# Snap a window that Set-WindowLayouts already inset into its zone
...
**See also:** [Snap-AllWindows](window.md#snap-allwindows), [Invoke-SingleZoneWindowSnap](window.md#invoke-singlezonewindowsnap)

## [Invoke-ReadyDesktopPass](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Invoke-ReadyDesktopPass.ps1)

- **Description:** Positions, resizes and snaps one virtual desktop as soon as `Wait-ForWorkspaceWindows` reports every entry on it stable - the wait's `-OnDesktopReady` callback as a named function, driven by the state object from `New-WorkspaceLayoutPipelineState`. The wait ends when the slowest window of the whole workspace has been stable for a second, and every other window has been sitting stable for seconds by then; this pass handles a ready desktop right away while the rest still load. It hands the WHOLE layout to `Set-WindowLayouts` restricted to that desktop (`-DesktopNumbers`, so duplicate keys are counted across desktops), narrows the claim set to every window the wait has confirmed stable anywhere so far (`Claims.WithCandidates` - a window still loading is never claimed; the wait matches by process OR title, so its per-entry window for a titled browser entry is often a different browser window, and a per-entry whitelist once left such entries unplaced), appends to the shared positioned-window tracking (`-KeepPositionedWindows`), then runs `Resize-PositionedWindows` and `Snap-AllWindows` for that desktop alone. Only what was actually placed counts as done: a `Configured` row goes into the pipeline's `PipelinedResults`, `PipelinedEntryKeys` and `Claims.Excluded`; a `Not Found` row is dropped so the pass after the wait places that entry and the shortfall is not counted twice. Any failure leaves the tallies untouched and the tail positions the desktop with the others. Its time is booked under `Position` and `Snap`, not `Wait`, and a running `Loading-Spinner` is paused around it.
- **Parameters:** -Pipeline, -ReadyDesktopNumber, -ReadyEntries, -StableWindowHandles, -AbandonedEntries
- **Usage:** `$onDesktopReady = { param($d, $e, $h, $a) Invoke-ReadyDesktopPass -Pipeline $pipeline -ReadyDesktopNumber $d -ReadyEntries $e -StableWindowHandles $h -AbandonedEntries $a }`

| Parameter              | Type           | Description                                                                                                                            |
| ---------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `-Pipeline`            | PSCustomObject | Mandatory. The layout pipeline state from `New-WorkspaceLayoutPipelineState`.                                                          |
| `-ReadyDesktopNumber`  | int            | Mandatory. The layout's 1-based desktop number the wait reported ready.                                                                |
| `-ReadyEntries`        | array          | `@{ LayoutEntry; Window }` for the desktop's stable entries.                                                                            |
| `-StableWindowHandles` | array          | Handles of EVERY window stable in the wait's poll, any entry, any desktop.                                                              |
| `-AbandonedEntries`    | array          | Layout entries the wait has abandoned so far; the layout pass gives them one search instead of the 1.5 s not-found ladder.             |

**See also:** [Set-WorkspaceWindowLayout](#set-workspacewindowlayout), [Wait-ForWorkspaceWindows](#wait-forworkspacewindows), [Move-StableWindowEarly](#move-stablewindowearly), [New-WorkspaceLayoutPipelineState](#new-workspacelayoutpipelinestate), [New-WindowClaimSet](#new-windowclaimset), [Set-WindowLayouts](#set-windowlayouts)
...
## [Invoke-VirtualDesktopOperation](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Invoke-VirtualDesktopOperation.ps1)

- **Description:** Runs one scriptblock against the third-party VirtualDesktop module's cmdlets and is the Window module's one seam to that module: every function that needs the desktop manager goes through it instead of calling `Switch-Desktop`, `Get-DesktopCount`, `Move-Window` and the rest directly, so the knowledge of how that module fails lives in exactly one place. The module is imported lazily (`Import-VirtualDesktopModule`); a machine without it gets one clear exception carrying the install command rather than a "command not found" from deep inside a caller. An RPC failure (`0x800706BA` and friends, classified by `Test-RpcUnavailableError` through the inner-exception chain and HRESULT) means this process holds COM proxies to a shell that is gone - a stale COM session after heavy desktop churn or an Explorer restart; the module compiles its proxies once per process and caches them, so the failure repeats forever unless they are reconnected. The seam reconnects the session with `Reset-VirtualDesktopState` and retries the operation with exponential backoff. Any other error (a window that closed mid-lookup, an index out of range) is the caller's business and is rethrown at once, untouched. When the attempts are exhausted the last error is thrown: a caller never sees a silent `$null` or `$false` from a broken session, which is how a stale count of one desktop once made every window move report "out of range" on a screen showing three. With `-Probe`, the live RPC endpoint is verified and repaired (`Get-RpcRetryPolicy -Probe`) before the first attempt - the preflight for a long sequence of desktop operations, not a per-call cost.
- **Parameters:** -Operation, -Label, -MaxAttempts (default: 3), -InitialDelayMs (default: 200), -Probe
- **Usage:** `$count = [int](Invoke-VirtualDesktopOperation -Operation { Get-DesktopCount } -Label 'counting desktops')`, `Invoke-VirtualDesktopOperation -Operation { $null = Switch-Desktop -Desktop 2 -ErrorAction Stop } -Label 'switching desktop'`, `Invoke-VirtualDesktopOperation -Operation { Get-DesktopCount } -Label 'desktop cleanup' -MaxAttempts 5 -InitialDelayMs 250 -Probe`

The scriptblock's output is the function's output. On top of the seam sit the verbs the rest of the repository uses: [Switch-VirtualDesktop](#switch-virtualdesktop) (switch, confirm, retry, session-reset fallback, window-cache clear), the two reads [Get-VirtualDesktopCount](#get-virtualdesktopcount) and [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex), [Get-WindowDesktopIndex](#get-windowdesktopindex), [Move-WindowToVirtualDesktop](#move-windowtovirtualdesktop) and [Ensure-VirtualDesktops](#ensure-virtualdesktops), plus [Remove-VirtualDesktops](system.md#remove-virtualdesktops) in the System module. `Focus-VirtualDesktop`, `Ensure-DesktopVisible`, `Snap-AllWindows`, `Apply-FancyZones`, `Set-WorkspaceWindowLayout`, `Confirm-WorkspaceWindowPositions`, `Move-Windows` and `Set-Wallpaper` call those verbs and no longer call the cmdlets or carry retry and recovery blocks of their own. `Reset-VirtualDesktopState`, `Reset-VirtualDesktopComProxy`, `Test-VirtualDesktopComHealth` and `Import-VirtualDesktopModule` remain exported - the module loader exports every file under `Functions` - but they are the seam's implementation; only this adapter and the System module's RPC repair tooling (`Repair-RpcServer`, `Restart-Explorer`, `Test-RpcServerHealth`) call them.

| Parameter         | Type        | Default                       | Description                                                                                                                    |
| ----------------- | ----------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `-Operation`      | scriptblock | -                             | Mandatory. The call to run; it invokes the VirtualDesktop cmdlets directly and its output is returned.                          |
| `-Label`          | string      | `virtual desktop operation`   | What the operation is, for the debug log and the RPC preflight message.                                                        |
| `-MaxAttempts`    | int         | `3`                           | How many times an RPC-classified failure is retried after a session reset (1-20).                                              |
| `-InitialDelayMs` | int         | `200`                         | Backoff before the second attempt, doubled each time (0-60000).                                                                |
| `-Probe`          | switch      | -                             | Verify and repair the live RPC endpoint before the first attempt. Use it in front of a sequence of desktop operations, not per call. |

```powershell
# A read: a stale session is reconnected and the count re-read instead of returning yesterday's number
$count = [int](Invoke-VirtualDesktopOperation -Operation { Get-DesktopCount } -Label 'counting desktops')

# A write, with the RPC preflight in front of a long sequence
Invoke-VirtualDesktopOperation -Operation { New-Desktop > $null } -Label 'creating desktops' -MaxAttempts 5 -InitialDelayMs 250 -Probe

# A non-RPC error is the caller's to handle - it is rethrown untouched on the first attempt
try { Invoke-VirtualDesktopOperation -Operation { Get-Desktop -Index 99 } -Label 'looking up a desktop' } catch { 'index out of range' }
```

**See also:** [Switch-VirtualDesktop](#switch-virtualdesktop), [Get-VirtualDesktopCount](#get-virtualdesktopcount), [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex), [Reset-VirtualDesktopState](#reset-virtualdesktopstate), [Test-RpcUnavailableError](helper.md#test-rpcunavailableerror), [Get-RpcRetryPolicy](helper.md#get-rpcretrypolicy)

## [Move-StableWindowEarly](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Move-StableWindowEarly.ps1)

- **Description:** Moves a window that just became stable to its layout entry's virtual desktop while the rest of the workspace is still loading - the `-OnWindowStable` callback of `Wait-ForWorkspaceWindows` as a named function, driven by the state object from `New-WorkspaceLayoutPipelineState` (its `Claims` and `DesktopOffset`). Desktop relocation is the slowest part of a layout pass, so it overlaps with the windows still loading instead of waiting until every one is ready. The move is skipped when the entry has no `DesktopNumber` and when the window is not this open's to claim (`Pipeline.Claims.TestClaimable`): a preserved alongside workspace's window, an existing window in alongside mode, or a window a per-desktop pass has already placed and snapped - every browser entry matches every browser window by process until the titles resolve, and one such match once carried a snapped VS Code window off to the browser's desktop. A failing move is swallowed; the pass after the wait moves the window again.
- **Parameters:** -Pipeline, -LayoutEntry, -Window
- **Usage:** `$onWindowStable = { param($entry, $window) Move-StableWindowEarly -Pipeline $pipeline -LayoutEntry $entry -Window $window }`

| Parameter      | Type           | Description                                                                                        |
| -------------- | -------------- | -------------------------------------------------------------------------------------------------- |
| `-Pipeline`    | PSCustomObject | Mandatory. The layout pipeline state from `New-WorkspaceLayoutPipelineState`.                      |
| `-LayoutEntry` | object         | Mandatory. The layout entry the wait matched the window to; its `DesktopNumber` is 1-based.        |
| `-Window`      | object         | Mandatory. The window object (`Handle`, `Title`) that became stable.                               |

**See also:** [Set-WorkspaceWindowLayout](#set-workspacewindowlayout), [Wait-ForWorkspaceWindows](#wait-forworkspacewindows), [Invoke-ReadyDesktopPass](#invoke-readydesktoppass), [New-WorkspaceLayoutPipelineState](#new-workspacelayoutpipelinestate), [Move-WindowToVirtualDesktop](#move-windowtovirtualdesktop)

## [Move-Windows](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Move-Windows.ps1)

- **Description:** Enumerates all visible application windows and moves each one to a specified virtual desktop (1-based; desktop 1 is the first). If the target desktop does not exist it is created automatically, and after the move pass focus is switched to the destination desktop. Use `-Current` to target the calling terminal's own desktop without knowing its number. Optional `-Monitor` repositions windows onto a target physical monitor in the same pass (resolved by `Resolve-TargetMonitor`), preserving each window's relative placement from its source monitor work area and clamping to the destination work area for safe multi-resolution placement. Each monitor placement is verified with `Wait-WindowRect` and re-applied once if the window did not hold its position, because `SetWindowPos` reports success for the call even when an external window manager moves the window straight back. Filtering by `-ProcessName` and/or `-WindowTitle` supports exact, wildcard, and regex matching (delegated to `Get-WindowHandle`); when both are given, windows matching either criterion are moved (OR logic). Windows already on the target desktop are skipped unless monitor repositioning is requested. Windows that could not be moved, or that would not stay on the target monitor, are reported in normal mode as well as under verbose logging. After the per-window pass, up to three convergence rounds re-enumerate the windows FRESH and re-check every eligible one against the target desktop (via `Get-WindowDesktopIndex`), retrying each straggler at most twice across rounds: a stale already-on-desktop read taken while a desktop collapse was still settling, the upstream `Move-Window` moving a sibling window of the same multi-window process (its fallback when the requested view cannot be moved), or a window that had no title yet - and so was not enumerated - while it loaded, all leave a window elsewhere while the first pass believed otherwise. Re-enumerating rather than re-checking the first pass's list is what catches the last case. The loop ends as soon as a round finds nothing off the target; a window it cannot recover is reported as a failure so the summary never shows a clean pass over a straggler.
...
Moves every visible window to a target virtual desktop and, optionally, a target physical monitor. The `-VirtualDesktop` and `-Current` parameters are mutually exclusive (separate parameter sets). Missing desktops are created on demand via `Ensure-VirtualDesktops`. Monitor targeting accepts a 1-based index (`1`, `2`), standardized labels (`Primary`, `Secondary`, `Monitor3`, ...), or an exact device name (for example `\\.\DISPLAY1`). System and shell windows (Program Manager, Start, Search, overlays, zero-size windows, etc.) are excluded. Under `Set-LogLevel Verbose`, a per-window trace plus a summary of moved, already-there, skipped, enumerated, eligible, and exclusion (skip-title / invalid-size) counts is printed. Every desktop read, move and switch goes through the Window module's seam - `Get-CurrentVirtualDesktopIndex` for `-Current`, `Get-VirtualDesktopCount` for the range check, `Get-WindowDesktopIndex` for each window, `Move-WindowToVirtualDesktop` for the move and `Switch-VirtualDesktop` for the final, confirmed switch to the destination - so a transient RPC failure such as `0x800706BA` is reconnected and retried by `Invoke-VirtualDesktopOperation`; this function calls no VirtualDesktop cmdlet directly and keeps only a plain `Invoke-WithRetry` (3 attempts, 200 ms) around each `Move-WindowToVirtualDesktop` call for a move that did not verify.
...
**See also:** [Reset-Windows](window.md#reset-windows), [Move-WindowToVirtualDesktop](window.md#move-windowtovirtualdesktop), [Switch-VirtualDesktop](window.md#switch-virtualdesktop), [Resolve-TargetMonitor](window.md#resolve-targetmonitor), [Wait-WindowRect](window.md#wait-windowrect)
...
- **Description:** Moves a window (identified by its handle) to the specified virtual desktop number. Sits on the Window module's seam: the current-desktop read, the desktop count, the target lookup and the `Move-Window` call all run through `Invoke-VirtualDesktopOperation`, so a stale COM session is reconnected and the call retried instead of a stale desktop count making every target "out of range". Requires the VirtualDesktop module (warns with the install command and returns `$false` without it). Uses 0-based desktop indexing internally; layout files use 1-based indexing, which is converted before this function is called.
- **Parameters:** -WindowHandle, -DesktopNumber, -Clock
...
A window already on the target desktop returns `$true` immediately (no COM move, no settle delay) - the common case, since workspace windows are desktop-moved from more than one code path. Otherwise it validates the target index against the live desktop count, resolves the destination desktop by its 0-based index, and moves the window there - each step through the seam, which rethrows a non-RPC error (a pinned or system window whose current desktop cannot be resolved) at once so the move path is still tried, and only lets an RPC failure abort after its own recovery. After a real move the result is verified immediately and then polled briefly (10ms steps, ~100ms budget) through `Wait-Until` instead of a fixed sleep, tolerating the transient `TYPE_E_ELEMENTNOTFOUND` error that the underlying COM interfaces often raise even on success; that poll reads and sleeps through the wait clock passed as `-Clock` (a real `New-WaitClock` by default), so a test injects a fake and asserts the exact poll count with no real waiting. The script-scoped `$script:LastMoveWindowToVirtualDesktopResult.Moved` reports whether a real move happened, so callers can skip their own settle delays on the fast path. Returns `$true` on confirmed success and `$false` on failure or an out-of-range index - always exactly one boolean: the upstream `Move-Window` cmdlet returns the Desktop object, which is discarded rather than leaked into the output, where it made every failed move truthy to callers (a two-element array is `$true` regardless of contents). If the VirtualDesktop module is missing it warns with install instructions and returns `$false`.

| Parameter        | Type   | Mandatory | Description                                 |
| ---------------- | ------ | --------- | ------------------------------------------- |
| `-WindowHandle`  | IntPtr | Yes       | The window handle (HWND) to move.           |
| `-DesktopNumber` | int    | Yes       | The target virtual desktop (0-based index). |
| `-Clock`         | object | No        | The wait clock (`New-WaitClock`) the post-move verification poll reads and sleeps through. Defaults to a real one; tests hand in a fake. |
...
**See also:** [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Get-WindowHandle](#get-windowhandle), [Ensure-VirtualDesktops](#ensure-virtualdesktops)

## [New-WindowClaimSet](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/New-WindowClaimSet.ps1)

- **Description:** Builds the one object that says which windows a layout pass may claim. A workspace open decides window ownership from facts that used to travel as five separate handle-set parameters through `Set-WorkspaceWindowLayout`, `Wait-ForWorkspaceWindows` and `Set-WindowLayouts`, each function re-deriving what they meant: `Existing` (windows on screen before the open started), `SkipExisting` (whether those are off limits - an `-Alongside` open adds to a screen other workspaces are using - or fair game, as on a plain open that owns the screen), `Protected` (windows of alongside workspaces a plain open preserves, never claimed), `Excluded` (windows a per-desktop pass already placed and snapped), `Candidates` (when set, the ONLY windows a pass may claim - the wait's stable windows, so a window still loading is never claimed; `$null` means no restriction) and `PinnedMap` (zone key to the window recorded in the applied layout, for handle pinning). The two rules every consumer needs are derived here once: `TestClaimable(handle)` is `$false` when the handle is protected, excluded, existing while `SkipExisting` is on, or not a candidate while candidates are restricted; `WaitExcluded()` is what the wait must skip entirely (protected windows, existing windows in alongside mode) and `WaitPreExisting()` what it counts as stable on first sight (existing windows in plain mode). `WithCandidates(handles)` returns a copy narrowed to those candidates with the same ownership rules. `Set-WorkspaceWindowLayout` builds the set once per open and hands it to `Wait-ForWorkspaceWindows` and `Set-WindowLayouts` as `-Claims`; its sets are live, and the per-desktop passes add every handle they place to `Excluded` as the open progresses.
- **Parameters:** -Existing, -SkipExisting, -Protected, -Excluded, -Candidates, -PinnedMap
- **Usage:** `$claims = New-WindowClaimSet -Existing $existingHandles -Protected $protection.WindowHandles`, `if ($claims.TestClaimable($window.Handle)) { ... }`, `$claims.WithCandidates($stableHandles)`

Every handle parameter accepts `IntPtr` values, integers, or objects with a `Handle` property; each is normalized into a `HashSet[IntPtr]` and zero handles are dropped. `-Candidates` is restrictive as soon as it is passed, even empty.

| Parameter       | Type      | Description                                                                                          |
| --------------- | --------- | ---------------------------------------------------------------------------------------------------- |
| `-Existing`     | object    | Handles of the windows that pre-existed the open.                                                    |
| `-SkipExisting` | switch    | Treat the existing windows as another workspace's (alongside mode).                                  |
| `-Protected`    | object    | Handles a plain open preserves ([Get-WorkspaceOpenProtection](workflow.md#get-workspaceopenprotection)). |
| `-Excluded`     | object    | Handles already placed by a per-desktop pass.                                                        |
| `-Candidates`   | object    | When given (even empty), the only handles a pass may claim.                                          |
| `-PinnedMap`    | hashtable | Zone key to pinned window record (`@{ Handle; ProcessId; ProcessName }`), or `$null`.                |

Returns a `PSCustomObject` with `Existing`, `SkipExisting`, `Protected`, `Excluded`, `Candidates` (`HashSet[IntPtr]` or `$null`), `HasCandidates`, `PinnedMap`, and the script methods `TestClaimable`, `WaitExcluded`, `WaitPreExisting` and `WithCandidates`.

**See also:** [Set-WindowLayouts](#set-windowlayouts), [Wait-ForWorkspaceWindows](#wait-forworkspacewindows), [Set-WorkspaceWindowLayout](#set-workspacewindowlayout), [New-WorkspaceLayoutPipelineState](#new-workspacelayoutpipelinestate)

## [New-WorkspaceLayoutPipelineState](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/New-WorkspaceLayoutPipelineState.ps1)

- **Description:** Creates the shared state of one workspace layout pass: what it lays out, with what, and what it has placed so far. `Set-WorkspaceWindowLayout` used to hold this in a dozen locals reached from two inline scriptblocks it handed to `Wait-ForWorkspaceWindows` as callbacks; those callbacks are the named functions `Move-StableWindowEarly` and `Invoke-ReadyDesktopPass` now, and this object is the one argument they take. It carries the immutable inputs of the pass (`LayoutConfig`, `MonitorInfo`, `MonitorConfig`, `DesktopOffset`, `DesktopCount`, `Alongside`, `Claims` from `New-WindowClaimSet`, `ZoneReset` - the scriptblock run when a snap exhausts its attempts - `RecordPhase` - the scriptblock booking elapsed time to a named phase - and `SpinnerActive`) and the live tallies the per-desktop passes fill in while the wait is still running and the tail after the wait then finishes: `PipelinedDesktops` (desktop number to `$true`), `PipelinedEntryKeys`, `PipelinedResults`, `PipelinedSnapFailures`, and `Claims.Excluded`, which receives every handle a pass placed so no later entry can carry it off.
- **Parameters:** -LayoutConfig, -Claims, -MonitorInfo, -MonitorConfig, -DesktopOffset, -DesktopCount, -Alongside, -ZoneReset, -RecordPhase, -SpinnerActive
- **Usage:** `$pipeline = New-WorkspaceLayoutPipelineState -LayoutConfig $layout -Claims $claims -MonitorInfo $monitors -MonitorConfig $config.Monitors -DesktopOffset 0 -DesktopCount 3 -ZoneReset $reset -RecordPhase $record`

| Parameter        | Type           | Default | Description                                                                                       |
| ---------------- | -------------- | ------- | ------------------------------------------------------------------------------------------------- |
| `-LayoutConfig`  | array          | -       | Mandatory. The layout entries to apply, tokens already resolved.                                  |
| `-Claims`        | PSCustomObject | -       | Mandatory. The window claim set for this open (`New-WindowClaimSet`).                             |
| `-MonitorInfo`   | array          | -       | The monitor list the layout resolves against.                                                     |
| `-MonitorConfig` | hashtable      | -       | The layout's monitor section (zone grids per monitor).                                            |
| `-DesktopOffset` | int            | `0`     | Virtual desktop offset for an alongside open.                                                     |
| `-DesktopCount`  | int            | `0`     | How many virtual desktops the layout needs.                                                       |
| `-Alongside`     | switch         | -       | Whether this is an alongside open.                                                                |
| `-ZoneReset`     | scriptblock    | -       | One string argument (the reason); resets FancyZones and re-applies the zone grids.                |
| `-RecordPhase`   | scriptblock    | no-op   | One string argument (the phase name); books elapsed time to that phase.                           |
| `-SpinnerActive` | switch         | -       | Whether a `Loading-Spinner` is running and must be paused around a per-desktop pass.              |

**See also:** [Set-WorkspaceWindowLayout](#set-workspacewindowlayout), [Move-StableWindowEarly](#move-stablewindowearly), [Invoke-ReadyDesktopPass](#invoke-readydesktoppass), [New-WindowClaimSet](#new-windowclaimset)
...
- **Description:** Restores a working VirtualDesktop session in place after the COM/RPC state has gone stale (the `0x800706BA` failure family an Explorer restart leaves behind). Two recovery layers: first `Reset-VirtualDesktopComProxy` reconnects the compiled type's cached static COM proxies to the current shell (the step that actually repairs a stale session - re-importing the module alone can never refresh them), then the module is removed, the module-scoped lazy-load cache (`$script:VirtualDesktopState`) is cleared, and the module is re-imported via `Import-VirtualDesktopModule`. When `Test-VirtualDesktopComHealth` is available, a live in-process roundtrip verifies the session actually works before success is reported. This is the recovery step of the Window module's seam, not something a caller runs by hand: `Invoke-VirtualDesktopOperation` runs it between attempts after classifying an RPC failure, and `Switch-VirtualDesktop` runs it once when a switch will not land.
- **Usage:** `Reset-VirtualDesktopState`, `if (Reset-VirtualDesktopState) { [void](Switch-VirtualDesktop -Index 0) }`

Returns a Boolean: `$true` only when the VirtualDesktop session is verified ready after the reset, otherwise `$false` (safe to retry after a delay - a restarted Explorer needs a moment to re-register its COM classes). Module removal failures are ignored (the module may not currently be loaded). Callers: the adapter (`Invoke-VirtualDesktopOperation` between retries, `Switch-VirtualDesktop` as the fallback after its switch attempts) and the System module's RPC repair tooling - `Repair-RpcServer` as its primary recovery step and `Restart-Explorer` proactively right after restarting the shell. `Snap-AllWindows`, `Focus-VirtualDesktop`, `Ensure-VirtualDesktops` and `Remove-VirtualDesktops` no longer call it themselves; they get the reconnect through the seam. The function stays exported because the module loader exports every file under `Functions`.

```powershell
# Reconnect the session by hand and only switch when it is verified ready again
if (Reset-VirtualDesktopState) { [void](Switch-VirtualDesktop -Index 0) }
```

**See also:** [Invoke-VirtualDesktopOperation](window.md#invoke-virtualdesktopoperation), [Switch-VirtualDesktop](window.md#switch-virtualdesktop), [Reset-VirtualDesktopComProxy](window.md#reset-virtualdesktopcomproxy), [Repair-RpcServer](system.md#repair-rpcserver)
...
- **Parameters:** -InsetPercent, -Tolerance, -DesktopNumbers
...
| `-DesktopNumbers` | Only the tracked windows on these desktops (the 1-based numbers `Add-PositionedWindow` recorded, offset folded in) are resized. `Set-WorkspaceWindowLayout` prepares one desktop for its snap this way while the others still load - a window on an already-snapped desktop sits at its full zone rect and must not be pulled back to the inset. Empty resizes every tracked window. |
...
## [Set-WindowForeground](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Set-WindowForeground.ps1)

- **Description:** Asks Windows to bring a window to the foreground, fire and forget - the plain `SetForegroundWindow` call through the Window module's native seam. It neither retries nor verifies: Windows may refuse (the caller is not in the foreground, the target is minimized) and that is reported as `$false`. Flows that must guarantee focus before injecting input use `Confirm-WindowForeground` instead. A zero handle is refused without calling Windows. Every module used to call the static native method directly; this keeps `WindowModule.Native` the one place `user32` is declared and lets a test mock a function instead of a compiled type.
- **Parameters:** -Handle
- **Usage:** `[void](Set-WindowForeground -Handle $window.Handle)`

**See also:** [Confirm-WindowForeground](#confirm-windowforeground), [Close-Window](#close-window)
...
| `-Claims`                | The window claim set for this pass ([New-WindowClaimSet](#new-windowclaimset)), or `$null` for "everything is claimable". One object carries the ownership facts that used to be six parameters: `Existing` (handles open before the layout run - detects pre-existing windows and skips already-correct positioning), `SkipExisting` (alongside mode - existing windows belong to a previous workspace and are dropped from every candidate list before claiming, so an entry left with none reports `Not Found`), `Protected` (a plain open preserves them - see [Get-WorkspaceOpenProtection](workflow.md#get-workspaceopenprotection); no entry can claim, move or recover one), `Candidates` (whitelist: when set, only these may be claimed - `Set-WorkspaceWindowLayout` narrows it to the windows the wait has confirmed stable when it positions one desktop while the rest still load, so a window still loading is never claimed whatever the title regex says, and such a pass searches each entry once and skips the not-found ladder), `Excluded` (blacklist: the windows its per-desktop passes already placed) and `PinnedMap` (from `CurrentLayout.txt`, keyed by `"<DesktopNumber>\|<Monitor>\|<Zone>"` to the recorded window `@{ Handle; ProcessId; ProcessName }` - the authoritative source for which duplicate-named window claims each zone: a still-live, process-matching record is reclaimed exactly; without one, candidates already on the entry's virtual desktop are preferred and geometry decides among them). |
...
| `-DesktopNumbers`         | Processes only the entries on these desktops (the layout's own 1-based `DesktopNumber`) while the WHOLE layout still drives duplicate-key detection and sort order - a key that appears once on that desktop but again on another must still claim exactly one window, which a layout subset could not know. |
| `-SkipEntryKeys`          | Entry keys (the `EntryKey` field every result row carries: desktop, monitor, zone or coordinates, process, title) to leave alone after duplicate-key detection over the whole layout. `Set-WorkspaceWindowLayout` passes the entries its per-desktop passes already placed when it finishes the rest. |
| `-KeepPositionedWindows`  | Appends to the positioned-window tracking instead of resetting it first (the default, so `Snap-AllWindows` sees exactly one call's windows). The passes of one workspace open share a single tracking set and pass this on *every* call - [Set-WorkspaceWindowLayout](#set-workspacewindowlayout) resets that set once for the whole open, before the wait, because its per-desktop passes fire from inside the wait and none of them is "the first". |
| `-AbandonedEntries`       | Layout entries the wait phase abandoned (`Wait-ForWorkspaceWindows`' `AbandonedEntries`: no window ever, no live process). They still get their `Not Found` row, but one search instead of the three-attempt ladder with its 0.5 s and 1 s waits - that ladder rides out transient title drift on a window that exists, and these have none. |
...
## [Set-WorkspaceRerunCommand](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Set-WorkspaceRerunCommand.ps1)

- **Description:** Records (or clears) the exact `Open-Workspace` invocation of the open in progress, for the failure-path respawn. `Open-Workspace` resolves its workspace names (the user may have picked them from the menu) and records the invocation here; when `Set-WorkspaceWindowLayout` escalates to `ReRun-LastCommand`, the fresh shell reruns precisely this command instead of scraping the shared PSReadLine history, where any other session may have written a newer line. `Open-Workspace` clears it when the open ends (its `finally` block, and before a terminating action exits the process).
- **Parameters:** -Command, -Clear
- **Usage:** `Set-WorkspaceRerunCommand -Command "Open-Workspace -Workspace 'WinuX'"`, `Set-WorkspaceRerunCommand -Clear`

**Why module state and not an environment variable.** The record used to be `$env:WORKSPACE_RERUN_COMMAND`, set for the whole open. A process environment variable is inherited by every application and terminal tab the open spawns - Windows Terminal hands the `wt.exe` caller's environment to command-line-created panes - so a standalone `Set-WorkspaceWindowLayout` escalation typed later into a project tab would have respawned the whole inherited workspace open. Module state lives and dies with this process alone. The elapsed-time base of `Open-Workspace` left the environment for the same reason (see [Open-Workspace](workflow.md#open-workspace)).

| Parameter  | Description                                                                 |
| ---------- | --------------------------------------------------------------------------- |
| `-Command` | The command line to record. Empty or whitespace clears the record.          |
| `-Clear`   | Clears the record; equivalent to an empty `-Command`.                       |
...
- **Parameters:** -WorkspaceName, -LayoutPath, -TimeoutSeconds, -SnapDelayMs, -DisableAutoWait, -PreCapturedExistingWindows, -DesktopOffset, -Alongside, -ProtectedWindowHandles, -PrepareOnly
- **Usage:** `Set-WorkspaceWindowLayout -WorkspaceName MyWorkspace`, `Set-WorkspaceWindowLayout -WorkspaceName OtherProject -DesktopOffset 2 -Alongside`, `Set-WorkspaceWindowLayout -LayoutPath C:\Users\<User>\MyLayouts\custom.psd1 -TimeoutSeconds 30`, `Set-WorkspaceWindowLayout -WorkspaceName MyWorkspace -DisableAutoWait`, `Set-WorkspaceWindowLayout -WorkspaceName MyWorkspace -PrepareOnly`
...
**The positioned-window tracking set belongs to the open, not to a layout pass.** `Set-WindowLayouts` records every window it places into the module-scoped set that [Resize-PositionedWindows](#resize-positionedwindows), [Snap-AllWindows](#snap-allwindows) and [Save-CurrentLayout](#save-currentlayout) then work through, and it resets that set on entry unless `-KeepPositionedWindows` says otherwise. A pipelined open passes that switch on *every* call - its per-desktop passes fire from inside the wait, so none of them is "the first" - which is why the open itself calls [Initialize-PositionedWindowTracking](#initialize-positionedwindowtracking) once, right before the wait. It has to: the set is process-scoped and survives between opens, so without that reset the second workspace opened in one shell still carries the first one's windows, and `Snap-AllWindows` re-resolves those dead handles by title and drives this workspace's windows to the previous workspace's coordinates against the wrong zone grid - six snap attempts and a FancyZones force-restart per stale entry, then an in-process retry. The in-process retries reset the set the ordinary way, through their un-flagged `Set-WindowLayouts` call.
...
| `-SnapDelayMs`                | int             | `10`    | Per-window delay on the simple-layout path (`Snap-AllWindows -All`). Unused in the workspace flow: nothing sleeps between positioning and the snap pass, and the positioned-window path verifies with `Wait-WindowRect` instead of fixed delays.                       |
...
| `-PrepareOnly`                | switch          | -       | Runs only the preamble - RPC probe, layout file and validation, virtual desktop resize, FancyZones zone layouts - and returns with outcome `Prepared`; nothing waits, moves or snaps, no snapshot is written, and the rerun markers are not read. `Open-Workspace` calls it this way before the launch actions - see below. A simple layout returns at once. |

**A plain open preserves live alongside workspaces.** When `-ProtectedWindowHandles` is set (or self-derived), the desktop resize target is raised to the highest desktop a protected window stands on - resolved live via `Get-WindowDesktopIndex`, never from a stored index, and never shrinking when occupancy cannot be resolved - so `Ensure-VirtualDesktops` (which removes desktops from the right, exactly where alongside lives) cannot delete a preserved workspace's desktops; a protected window inside the plain layout's own desktop range is reported as an overlap warning. Protected windows are refused by the claim set (`New-WindowClaimSet -Protected`), so `Move-StableWindowEarly` never moves one and no `Set-WindowLayouts` entry can claim one; they are skipped by browser first-tab normalization (in both the unresolved-entry scan and the normalization loop), excluded from plain-mode verification via `Confirm-WorkspaceWindowPositions -ExcludeWindowHandles`, and the success-path `Save-CurrentLayout` runs with `-PreserveOtherSections` so the preserved workspaces keep their snapshot sections. Simple layouts (`Fullscreen`/`Empty`) deliberately ignore protection: they are global-by-design gestures.

**Alongside narrows what the layout may touch**, not only where it lands: the claim set is built with `SkipExisting` on (`New-WindowClaimSet -Existing $preOpenCapture -SkipExisting`), so every window captured before the open belongs to whichever workspace is already running and is refused by `Set-WindowLayouts` and by the wait alike. Two things follow from that rule.
...
On entry the function runs a live RPC preflight (`Get-RpcRetryPolicy -OperationLabel "applying layout" -Probe`): the probe verifies this session's VirtualDesktop COM state in-process, so a session whose proxies went stale after an Explorer restart is detected and repaired (via `Repair-RpcServer` / `Reset-VirtualDesktopState`) before any desktop reconfiguration begins. After that preflight every desktop operation the function makes goes through the Window module's seam - `Ensure-VirtualDesktops`, `Get-VirtualDesktopCount`, `Get-WindowDesktopIndex` and `Switch-VirtualDesktop` - so it calls no VirtualDesktop cmdlet directly and carries no retry or recovery block of its own; a stale session mid-run is reconnected by `Invoke-VirtualDesktopOperation`. On success Windows Terminal is refocused just before the success banner so output is not buried behind workspace windows. On snap or verification failure the position → snap → verify pipeline is first retried in-process up to 2 times (in `-Alongside` mode too): the existing-window snapshot is refreshed so already-correct windows are skipped by the position check (in `-Alongside` mode it stays the original pre-open capture, since there it marks the other workspace's windows), and verification runs against the full layout config so windows an aborted snap pass never reached are covered. `-Alongside` is verified too, but scoped twice over: only the entries this pass actually placed a window for (rebuilt from the `LayoutEntry` field on the `Set-WindowLayouts` results), matched only against windows this open created (the pre-open snapshot is passed as `Confirm-WorkspaceWindowPositions -ExcludeWindowHandles`). Another workspace's windows can therefore neither be checked nor mistaken for this workspace's, which is what the earlier blanket skip was avoiding - at the cost of reporting unconditional success, so a starved or mispositioned alongside pass was never retried and was persisted by `Save-CurrentLayout` as the truth, pinning the next open to those wrong zones. When nothing at all was placed, verification is skipped: no retry can conjure windows, and the shortfall warning below is the report. Only when the in-process retries are exhausted does the function record the failed window marker (now informational only), force-start FancyZones, and rerun via `Initialize-WorkspaceWindowLayoutRerun -WindowOnlyRetry` / `ReRun-LastCommand` in a window-only mode that preserves virtual desktops and caches, always reapplies FancyZones monitor layouts, and applies the full layout config - idempotent skips keep it cheap (still capped at 2 auto-reruns). The counter behind that cap and the window-only markers reach the respawned shell through the User-scope mirror (`Get-WorkspaceRerunMirror` / `Set-WorkspaceRerunMirror`), never through the process environment: Windows Terminal generates a new environment block for every session, so the new shell's process copies are the stamped mirror values themselves and are skipped in favour of the mirror (reading them as plain used to throw on the `[int]` cast of the counter and to leave the respawned run unaware it was a retry). The rerun command is taken from [Get-WorkspaceRerunCommand](#get-workspacereruncommand) (the in-process record `Open-Workspace` keeps through `Set-WorkspaceRerunCommand`) when present, instead of scraping PSReadLine history; it is never read from the environment, so a tab spawned by an earlier open cannot hand this escalation a stale command. Auto-rerun is disabled in `-Alongside` mode. `CurrentLayout.txt` is written only on the success paths, so a failed/rerunning attempt never overwrites the last good snapshot.

**Desktops are positioned and snapped as they become ready, not after the slowest window.** The wait ends when the slowest window of the whole workspace (VS Code with a workspace, Claude Desktop) has been stable for a second, and every position and snap second used to be paid after that although the other windows had been sitting stable for seconds. `Wait-ForWorkspaceWindows` now reports each virtual desktop whose entries are all stable while others still load (`-OnDesktopReady`), and that desktop is handled right then, inside the wait, by [Invoke-ReadyDesktopPass](#invoke-readydesktoppass): `Set-WindowLayouts` over the whole layout restricted to that desktop's entries (`-DesktopNumbers`, so duplicate keys are still counted across desktops), claiming only windows the wait has confirmed stable so far (the claim set narrowed with `Claims.WithCandidates` - every stable window, not the wait's per-entry match: the wait matches by process OR title, so its window for a titled browser entry is often a different browser window, and a per-entry whitelist left such entries unplaced), appended to the one positioned-window tracking the whole open shares (`-KeepPositionedWindows`), then `Resize-PositionedWindows` and `Snap-AllWindows` for that desktop alone. Only entries that were actually placed count as done, and the early move ([Move-StableWindowEarly](#move-stablewindowearly), the wait's `-OnWindowStable` callback) never touches a window a per-desktop pass has already placed (every browser entry matches every browser window by process until the titles resolve, and one such match once carried a snapped VS Code window off to the browser's desktop). A catch-all entry (process, no title) is run again in the tail when the unplaced windows of its process **outnumber** the entries with the same key the tail still has to place - so the second VS Code window of a two-project open is placed too, with the placed one excluded - and only as many placed entries as there are surplus windows are re-run. The count matters: a browser layout repeats one catch-all key across every desktop, and re-running a placed entry whenever *any* unplaced window of its process existed (the rule until 0.1.57) re-queued every browser entry the early desktops had placed while the later desktops were still loading; those entries ran first in the tail, claimed the later desktops' windows as a second copy in every zone, and the last entries found nothing - a starved first pass that only the in-process retry straightened out. In the same spirit `Set-WindowLayouts`' duplicate-key claim now prefers a candidate already on the entry's desktop, so the tail keeps the early move's assignment instead of shuffling windows between desktops in layout order. The tail after the wait finishes every entry the per-desktop passes did not place (`-SkipEntryKeys` for the placed ones, the claim set's `Excluded` for their windows, which also leave the wait snapshot and the tracking so no title fallback claims one and no resize pulls a snapped window back to its inset), and the first-open normalization and browser first-tab passes skip what was already placed. Verification stays global, the in-process retries run the full layout as before, and the phase clock books the in-wait work under `Position` and `Snap`. Cold opens save roughly the position and snap time of every desktop but the slowest one; warm reruns are unchanged. `WorkspaceLayoutPipelining = $false` in the configuration restores the strictly sequential order. The two callbacks and the tail share one state object, [New-WorkspaceLayoutPipelineState](#new-workspacelayoutpipelinestate) (the layout, monitors, offset, claim set, zone reset and phase clock as inputs; the placed desktops, entry keys, results and snap failures as live tallies), and one claim set, [New-WindowClaimSet](#new-windowclaimset), built once per open from the pre-open capture, the alongside flag, the protected handles and the pinned-handle map - the five handle sets that used to travel as separate parameters. Inside the snap pass, a window that exhausts keyboard and shift-drag snapping triggers this function's FancyZones reset (handed to `Snap-AllWindows` as `-ZoneReset`) and a second round for that same window before anything is recorded as failed - see [Snap-AllWindows](window.md#snap-allwindows).

**The layout preamble runs before the launch actions.** With `-PrepareOnly` the function runs the RPC probe, loads and validates the layout file, resizes the virtual desktops and applies the FancyZones zone layouts, then returns with outcome `Prepared` - no wait, no positioning, no snap, no snapshot write, and the one-shot rerun markers are not read (reading consumes them, and the full run that follows has to see them). [Open-Workspace](workflow.md#open-workspace) calls it that way before its launch actions when `WorkspaceLayoutPrepareEarly` is on (the shipped default), so that preamble runs on an idle machine instead of competing with a dozen starting applications - 3.4 s under that load against 0.2 s idle, the RPC probe runspace alone 1.7 s. The ordinary layout action then finds the desktop count already right and the zone layouts already in `applied-layouts.json`, and both phases collapse to their checks. A simple layout returns at once, and a window-only retry is never prepared (its point is to leave the desktops alone). The prepare run publishes its own phase record; `Open-Workspace` folds it into the benchmark row, so `Get-WorkspaceBenchmark` still shows the `Desktops` and `FancyZones` seconds wherever they were spent, and the preparation itself appears in the `Actions` column as `Set-WorkspaceWindowLayout -PrepareOnly`, counted as layout time.

**The wait credits what it can see.** Two trims that leave the stability rules alone. A window that existed before the open has nothing to stabilize: on a plain open the pre-open capture reaches [Wait-ForWorkspaceWindows](#wait-forworkspacewindows) through the claim set (`-Claims`, whose `WaitPreExisting()` lists it) and such a window counts as stable the first time an entry matches it. On an alongside open the same capture lands in the claim set's `WaitExcluded()` instead - those windows belong to another workspace and the layout pass refuses them, so waiting a second for one only to refuse it wasted the second and hid a real shortfall; protected windows are excluded on a plain open for the same reason, and an entry that only ever matches excluded windows is abandoned after the process-absent grace period rather than the timeout. Windows the launch actions created earn the stability floor exactly as before - a first cut also credited them the time they had been visible before the wait started, and handed a 2-second-old VS Code window over while VS Code was replacing it (see the `Confirm-WorkspaceWindowPositions` desktop rule and `Resolve-PositionedWindowHandle` for the two safety nets that incident also hardened). Entries the wait abandoned come back as `AbandonedEntries` and go to `Set-WindowLayouts -AbandonedEntries`, which gives them one search instead of the 0.5 s + 1 s not-found ladder - inside the per-desktop passes too, where the ladder would have delayed the other desktops. After the layout is saved, `Visualize-Layouts -LayoutPath` renders the file that was just applied instead of rescanning the whole `Layouts` tree for it.

A few passes between waiting and positioning are kept deliberately narrow. First-open normalization resizes only the windows this open created (pre-existing windows elsewhere on the machine are left untouched). Browser first-tab normalization (Ctrl+1) skips windows already showing a title some browser layout entry wants, and touches pre-existing browser windows only when a browser entry's title currently matches no window; it no longer probes tab counts via a UIA tree walk. The pre-snap resize (`Resize-PositionedWindows`) runs with the module's default 20px tolerance so windows that self-adjust by a pixel (terminal cell rounding, min-size constraints, DPI rounding) converge instead of being re-positioned on every open. And a virtual desktop count mismatch is delta-resized with a single `Ensure-VirtualDesktops` call (it grows and shrinks) instead of removing all desktops and recreating them. Nothing sleeps between positioning and the snap pass either: `Set-WindowLayouts` settles every `SetWindowPos`, the pre-snap resize re-reads each rect, and the snap verifies with `Wait-WindowRect`.
...
- **Parameters:** -All, -CurrentDesktopOnly, -WindowHandles, -SnapDelayMs, -DesktopOffset, -DesktopCount, -DesktopNumbers, -ZoneReset
- **Usage:** `Snap-AllWindows`, `Snap-AllWindows -All`, `Snap-AllWindows -All -CurrentDesktopOnly`, `Snap-AllWindows -All -SnapDelayMs 100`, `Snap-AllWindows -DesktopOffset 2 -DesktopCount 3`, `Snap-AllWindows -DesktopNumbers 2 -ZoneReset $resetFancyZones`
...
In the workspace (positioned-windows) flow, windows are grouped by desktop and processed in positioning order. Desktop switches go through [Switch-VirtualDesktop](#switch-virtualdesktop), which confirms the desktop is showing, retries, falls back to a session reset (`Reset-VirtualDesktopState`) when the switch still has not landed and clears the window cache once it has; the pass reads where it started with `Get-CurrentVirtualDesktopIndex` and each window's desktop with `Get-WindowDesktopIndex`, and carries no desktop-switch retry or recovery block of its own. After each transition the monitor/window caches are refreshed, each window is revalidated and realigned to the target desktop, and stale handles are recovered via title + process fingerprint matching (`Resolve-PositionedWindowHandle`). Re-positioning uses the same shared inset resize path as `Set-WindowLayouts` and `Resize-PositionedWindows`, and focus is verified immediately before key injection via `Confirm-WindowForeground`. Keyboard-snap and shift-drag results are verified by polling the window rect via `Wait-WindowRect` (the time budget grows on each retry) instead of a fixed sleep and single check, so a snap that lands quickly returns immediately and the expensive fallbacks only trigger when the budget is genuinely exhausted. Target desktop indices are computed through `ConvertTo-InternalDesktopIndex` so `DesktopOffset` is honored consistently.

At start the function requires a verified FancyZones ready state, and during long snap loops it re-checks FancyZones liveness once per desktop pass (rather than per window) and attempts a restart if the process disappears; if FancyZones cannot be recovered it aborts early. The multi-zone attempt budget itself lives in [Invoke-MultiZoneWindowSnap](window.md#invoke-multizonewindowsnap) (`Win+Up`, shift-drag fallback, three attempts), the twin of `Invoke-SingleZoneWindowSnap`.

**A window that exhausts its attempts resets the zone grid and gets a second round, before the pass moves on.** A window that failed keyboard AND shift-drag snapping three times is not a stubborn window - it is the signal that the zone grid it was snapped against is wrong (FancyZones wedged, a stale grid, a missed layout), the very signal the circuit breaker used to wait three windows for while the rest of the pass was snapped against that same grid and the caller then paid a full position-snap-verify cycle. So the pass acts on the first one: it releases the keyboard modifiers and the drag's mouse button, runs `-ZoneReset` (Set-WorkspaceWindowLayout hands over its own FancyZones reset - force-restart plus `Apply-FancyZones -Force`; a standalone call without one gets a FancyZones force-restart alone, having no monitor configuration to re-apply zones with), re-confirms the window's desktop (`Switch-VirtualDesktop` - the shortcut fallback of the re-apply may end elsewhere), refreshes the window and monitor caches, re-insets the window and runs the whole attempt budget again for that SAME window. Only a window that fails the second round is recorded in `FailedWindows`, with `after 6 attempts (unverified position, zone grid reset once)` in its error, and the pass then continues with the remaining windows so one window cannot strand everything after it at its inset size. Two resets per pass at most: two resets that did not help are systemic, so the third exhausted window records without one and the circuit breaker (three failures) hands the pass back to `Set-WorkspaceWindowLayout`'s retry and rerun escalation, which are unchanged. A desktop that cannot be re-confirmed after the reset records the failure without a second round.

After processing, the active desktop is left on the last one snapped - returning the user to the first desktop is delegated to `Focus-VirtualDesktop` (the final workspace action), keeping switch-and-focus logic in one place (DRY). With `-DesktopNumbers`, only the tracked windows on those desktops (the 1-based numbers `Add-PositionedWindow` recorded, offset folded in) are processed and swept - `Set-WorkspaceWindowLayout` uses it to snap one desktop as soon as its windows are stable while the rest of the workspace is still loading.
...
| `-DesktopNumbers`     | int[]  | -       | Restricts the positioned-windows pass (and its post-pass sweep) to the tracked windows on these desktops. Empty processes every tracked window.   |
| `-ZoneReset`          | scriptblock | -  | Run once for a window that exhausted its snap attempts, before that window's second round; receives the reason as its one argument. Restart-only recovery when omitted. |
...
# Snap one desktop's tracked windows, resetting the zone grid through the caller's scriptblock when one exhausts its attempts
...
**See also:** [Switch-VirtualDesktop](window.md#switch-virtualdesktop), [Set-WorkspaceWindowLayout](window.md#set-workspacewindowlayout), [Invoke-SingleZoneWindowSnap](window.md#invoke-singlezonewindowsnap), [Invoke-SingleZoneWindowPlacement](window.md#invoke-singlezonewindowplacement), [Resize-Windows](window.md#resize-windows), [Get-InsetWindowBounds](window.md#get-insetwindowbounds)

## [Switch-VirtualDesktop](https://github.com/IvanPavlak/WinuX/blob/master/Windows/PowerShell/Modules/Window/Functions/Switch-VirtualDesktop.ps1)

- **Description:** Brings a virtual desktop on screen and confirms it is showing. Switching desktops is asynchronous and, in a long-running shell, silently unreliable: a `Switch-Desktop` against a stale COM proxy no-ops without an error. So this does what every caller used to do by hand - it switches through `Invoke-VirtualDesktopOperation`, polls `Get-CurrentVirtualDesktopIndex` until the target desktop is the one showing (up to `-TimeoutMs` per attempt, every `-PollIntervalMs`), retries up to `-MaxAttempts` times, and when the desktop still has not landed reconnects the VirtualDesktop session with `Reset-VirtualDesktopState` and tries once more. After a successful switch the window cache is cleared (`Clear-WindowCache`), because handles enumerated before it describe the previous desktop. A desktop that is already showing is reported as switched without touching the desktop manager. Returns `$true` when the desktop is showing and `$false` when it never landed; it throws only when the desktop manager cannot be reached at all (module missing, or RPC still dead after recovery).
- **Parameters:** -Index, -TimeoutMs (default: 750), -PollIntervalMs (default: 10), -MaxAttempts (default: 3), -Clock
- **Usage:** `Switch-VirtualDesktop -Index 2`, `if (-not (Switch-VirtualDesktop -Index 0)) { Write-LogError "Desktop 1 did not come on screen" }`, `Switch-VirtualDesktop -Index 1 -TimeoutMs 1000 -MaxAttempts 5`

This is the one switch verb in the repository. `Focus-VirtualDesktop`, `Ensure-DesktopVisible`, `Ensure-VirtualDesktops`, `Snap-AllWindows`, `Apply-FancyZones`, `Set-WorkspaceWindowLayout`, `Move-Windows` and `Set-Wallpaper` call it instead of `Switch-Desktop` and no longer carry a poll, a retry loop or a `Reset-VirtualDesktopState` fallback of their own - the switch-then-poll block each of them used to hand-roll is this function. Transient COM errors while polling read as "not yet", not as failure. Each attempt's poll is a `Wait-Until` that reads time and sleeps through the wait clock passed as `-Clock` (a real `New-WaitClock` by default), so a test injects a fake clock and asserts the exact number of polls per attempt with no real waiting. The index is 0-based; layout-file desktop numbers are 1-based and converted with `ConvertTo-InternalDesktopIndex` first.

| Parameter         | Type   | Default | Description                                                                 |
| ----------------- | ------ | ------- | --------------------------------------------------------------------------- |
| `-Index`          | int    | -       | Mandatory. The 0-based index of the desktop to show (0-255).                |
| `-TimeoutMs`      | int    | `750`   | How long each attempt waits for the switch to land.                         |
| `-PollIntervalMs` | int    | `10`    | Delay between checks while waiting. `0` spins.                              |
| `-MaxAttempts`    | int    | `3`     | Switch attempts before the session reset is tried.                          |
| `-Clock`          | object | -       | The wait clock (`New-WaitClock`) each attempt's wait reads and sleeps through. Defaults to a real one; tests hand in a fake. |

```powershell
# Switch and act only once the desktop is really showing
if (Switch-VirtualDesktop -Index 2) {
    Send-FancyZonesLayoutShortcut -LayoutNumber 1
}

# Give a loaded machine more time per attempt
[void](Switch-VirtualDesktop -Index 0 -TimeoutMs 1500 -MaxAttempts 5)
```

**See also:** [Invoke-VirtualDesktopOperation](#invoke-virtualdesktopoperation), [Get-CurrentVirtualDesktopIndex](#get-currentvirtualdesktopindex), [Ensure-DesktopVisible](#ensure-desktopvisible), [Focus-VirtualDesktop](#focus-virtualdesktop), [Clear-WindowCache](#clear-windowcache)
...
- **Parameters:** -Targets, -WaitForWriteAfterUtc, -TimeoutMs, -PollIntervalMs, -AppliedLayoutsPath, -Clock
...
| `-Clock`                | object   | -       | The wait clock (`New-WaitClock`) the last-write poll and the parse retry read and sleep through. Defaults to a real one; tests hand in a fake. |
...
Returns a `PSCustomObject` with `Healthy` (bool), `TimedOut` (bool), and `Error` (innermost failure message plus HRESULT, or `$null`). A healthy warm probe completes in milliseconds. Used by `Test-RpcServerHealth -Probe` as the live endpoint check and by `Reset-VirtualDesktopState` to verify a reset actually produced a working session; both are the implementation of the Window module's seam (`Invoke-VirtualDesktopOperation`) and the System module's RPC repair tooling, which are the only callers.
...
**See also:** [Close-Window](#close-window), [Wait-WindowsClosed](#wait-windowsclosed), [Wait-BrowserWindowsClosed](system.md#wait-browserwindowsclosed)
...
A workspace-orchestration helper (called by `Set-WorkspaceWindowLayout` / `Set-WindowLayouts`) that blocks until the apps for a layout have actually finished initializing, so windows are positioned only once they are stable. An entry with a process pattern only ever matches windows of that process: the title pattern is a redundancy within it (a browser window counts by process while its title is still loading), never a way for a window of another process to match - a browser entry's `\bSEUP\b` once matched `seup-ui - Visual Studio Code`. While waiting it keeps the Windows Terminal topmost to avoid flicker when browser windows steal focus, then restores normal z-order on exit. Returns a hashtable with `Success` (bool - `$false` when the wait timed out or any entry was abandoned), `WindowStates` (handle → position/size snapshot; on failure it still carries the windows that did stabilize, feeding downstream title-drift fallbacks), `Abandoned` (descriptions of entries abandoned by the fail-fast) and `AbandonedEntries` (the same entries as layout entries, for `Set-WindowLayouts -AbandonedEntries`), plus `ReadyDesktops` (the desktops `-OnDesktopReady` fired for).

**Per-desktop readiness.** With `-OnDesktopReady`, every poll in which the wait as a whole is still going checks each virtual desktop (the entries' `DesktopNumber`, default 1): when all of a desktop's entries are stable in that poll - abandoned ones count as done - and at least one is stable, the callback fires once for that desktop with the desktop number and `@{ LayoutEntry; Window }` for each stable entry. From then on those entries count as ready whatever their windows do (the caller is positioning and snapping them on purpose, which changes their rects), and the terminal's topmost flag is dropped for the duration of the callback so the caller's drag snaps cannot land on it. It never fires in the poll that completes the wait, so a workspace whose desktops all finish together pays nothing for it. A titled entry counts towards its desktop's readiness only on a window whose title matches its pattern; found by process alone keeps the wait going but does not hand the desktop over, so a per-desktop pass is not sent after a window that is not there yet. The candidate handles handed over are every window stable in that poll, whichever entry matched it. `Set-WorkspaceWindowLayout` uses it to position and snap each desktop while the slower windows on the others are still loading. The callback's fourth argument is the list of layout entries abandoned so far, so that pass can skip its not-found ladder for them.

**Stability credit and excluded windows.** Both come from the `-Claims` object ([New-WindowClaimSet](#new-windowclaimset)); the wait derives two sets from it and never re-reads the underlying handle sets itself. One kind of window carries stability in with it: a window in `Claims.WaitPreExisting()` (a plain open's pre-open capture) has nothing to stabilize and counts as stable the first time an entry matches it. Windows the launch actions created earn the floor whatever they looked like before the wait started - an application's first window can be replaced within seconds of its launch (VS Code hands its window from the launcher to the main process 2 to 3 s in, same title and size), and the floor is what catches that. Windows in `Claims.WaitExcluded()` (an alongside open's pre-existing windows, a plain open's protected windows) never match an entry at all, since the layout pass may not use them either; an entry whose every match so far was excluded is abandoned after `ProcessAbsentGraceSeconds` like an entry whose process never appeared, so a starved alongside entry costs the grace period instead of the timeout. Under `Set-LogLevel Verbose` a credited window logs `stable on first sight (pre-existing window, 1s credited)`. The poll itself reads time and sleeps through the wait clock passed as `-Clock` (a real `New-WaitClock` by default), so a test injects a fake clock and drives the whole wait, callbacks included, with no real waiting.
...
| `-OnDesktopReady`               | scriptblock | -       | Callback fired once per virtual desktop as soon as every entry on it is stable or abandoned while another entry is still pending, receiving the desktop number, `@{ LayoutEntry; Window }` per stable entry, and the handles of every window stable in that poll, plus the layout entries abandoned so far. Never fires in the poll that completes the wait. |
| `-Claims`                       | PSCustomObject | -   | The window claim set of this open ([New-WindowClaimSet](#new-windowclaimset)), or `$null`. Its `WaitPreExisting()` are windows that existed before a plain open - one counts as stable the first time an entry matches it, nothing to stabilize; its `WaitExcluded()` are windows that never match an entry (an alongside open's pre-existing windows, a plain open's protected windows) - an entry whose only matches are excluded is abandoned after `ProcessAbsentGraceSeconds`. |
| `-Clock`                        | object      | -       | The wait clock (`New-WaitClock`) the poll reads and sleeps through. Defaults to a real one; tests hand in a fake. |
...
- **Parameters:** -WindowHandle, -ExpectedX, -ExpectedY, -ExpectedWidth, -ExpectedHeight, -TolerancePx (default: 20), -TimeoutMs (default: 300), -PollIntervalMs (default: 15), -Clock
...
Used by `Snap-AllWindows` to verify keyboard-snap and shift-drag results, with a time budget that grows on each retry. Position AND size must both match within `TolerancePx` per edge before the poll succeeds. Polling stops early when the handle becomes unreadable (window closed or recreated), since it can no longer succeed. Returns a `PSCustomObject` with `Verified` (bool - `$true` once the rect matched within the budget), the last observed `X`/`Y`/`Width`/`Height` (`$null` when the rect was never readable), and `ElapsedMs` (how long the poll ran). The poll is a `Wait-Until` that reads time and sleeps through the wait clock passed as `-Clock` (a real `New-WaitClock` by default), so a test injects a fake clock and asserts the exact poll count with no real waiting.
...
- **Parameters:** -Window, -TimeoutMilliseconds (default: 1500), -PollIntervalMilliseconds (default: 250), -Clock
- **Usage:** `Wait-WindowsClosed -Window $postedWindows`, `Wait-WindowsClosed -Window $postedWindows -TimeoutMilliseconds 3000`

An empty result means they all closed. `Clear-WindowCache` runs before each poll - without it the check keeps reading the same pre-close snapshot and every window looks like it refused. Matching is by handle only: a window that closed and was replaced by a new window of the same application is a different window, and this deliberately does not wait for that one. An empty input collection returns immediately without enumerating anything. Used by [Close-Workspace](workflow.md#close-workspace) to tell "closed" from "refused" before it sweeps empty virtual desktops. The poll is a `Wait-Until` that reads time and sleeps through the wait clock passed as `-Clock` (a real `New-WaitClock` by default), so a test injects a fake clock and asserts the exact poll count with no real waiting.
...
2. `Set-WindowLayouts` pre-scans the layout for duplicate `(ProcessName, WindowTitle)` pairs - each entry claims exactly one distinct window handle so they end up in different zones; among the unclaimed candidates a window recorded for the zone in `CurrentLayout.txt` wins, then one already on the entry's virtual desktop, then the one whose current bounds are closest to the zone
