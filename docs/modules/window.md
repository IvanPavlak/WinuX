...
Only `-MonitorConfig` is mandatory. With no `-DesktopNumber`, it applies layouts across all virtual desktops (using the VirtualDesktop module, when available) and returns to the starting desktop. Layout names resolve to shortcut numbers (0-9) via `LayoutNumbers` in `Configuration.psd1`. Returns a result array (one row per monitor/desktop) with a `Status` such as `Shortcut Sent`, `Already Applied`, `Monitor Not Found`, or `Failed`.
...
When multiple live windows match the same `ProcessName`/`WindowTitle`, candidates are scored by how closely their actual bounds match the expected bounds and the best-matching one is selected; a `$claimedHandles` set guarantees each duplicate-keyed layout entry claims a unique window so identical titles (e.g. two browser windows) are never misassigned. A title-drift fallback handles non-browser apps whose captions change at runtime (e.g. new Outlook / `Olk` titling its window after the selected folder): when strict process-and-title matching finds nothing and exactly one unclaimed process window remains, that window is accepted rather than false-failing. Candidates on the expected virtual desktop are preferred, falling back to cross-desktop candidates only when none exist there.
...
Lazy-loads the `VirtualDesktop` module if it is not already imported, then counts the existing desktops via `Get-DesktopList` and returns that count, which is the 0-based index of the next position after all current desktops (e.g. with 2 desktops at indices 0 and 1, it returns `2`). If the `VirtualDesktop` module is unavailable or an error occurs, it safely returns `0`.
...
Validates the target index against the available desktop count, resolves the destination desktop by its 0-based index, and moves the window there. After moving it verifies the window actually landed on the requested desktop, tolerating the transient `TYPE_E_ELEMENTNOTFOUND` error that the underlying COM interfaces often raise even on success. Returns `$true` on confirmed success and `$false` on failure or an out-of-range index. If the VirtualDesktop module is missing it warns with install instructions and returns `$false`.
...
Positions a single window by handle and pixel coordinates. Automatically restores maximized or snapped windows to a normal state before repositioning, then applies the new bounds. Returns `$true` on success and `$false` on failure.
...
| `-MinimumStableDurationSeconds` | double      | `1`     | Seconds a window (and then all windows collectively) must remain stable before being considered fully loaded.                                                      |
...
│  ├── 3. Focus browser first tabs (multi-tab windows only)           │
...
│  └── 10. Auto-rerun with targeted resize on failed snap/verify      │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
