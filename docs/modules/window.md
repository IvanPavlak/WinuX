...
**Which layout set is used** (parameter set `ByWorkspace` only - `-LayoutPath` is always taken as given) resolves in this order:

1. `LayoutMachineTypeOverrides[<detected machine type>]` - when that entry is a non-empty string, it replaces the machine type for layout resolution: the folder becomes `Layouts/<value>/` and the file `<WorkspaceName>_<value>.psd1`. Nothing else keyed by machine type changes (base paths, symbolic links, wallpapers, themes all keep using the real type).
2. `SmallDisplayMachineType` - when no override applies and the primary display is at most 3000px wide, that machine type's layout set is used instead.
3. The detected machine type's own layout set.

The manual override is checked first on purpose: it exists so a machine can be driven on a monitor setup its layouts were never authored for (a desktop moved away from its multi-monitor rig, a temporary single screen) without editing or losing that layout set. Author the new geometry in its own folder, point the override at it, and clear the entry to switch back - having that decision silently overruled by a display-width guess is exactly the surprise the ordering removes. When the override is active and a workspace has no layout file in the redirected set, the "No layout configuration found" warning names the active set and the path it expected, rather than leaving a missing file unexplained; there is no silent fallback to the machine's own layouts, because those describe a monitor setup that is not currently attached.
