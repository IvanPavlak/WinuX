...
- **Parameters:** -LayoutConfig, -ConfigPath, -MonitorInfo, -MonitorConfig, -ExistingWindowHandles, -ExpectedWindowState, -DesktopOffset, -SkipExistingWindows, -PinnedHandleMap, -ProtectedWindowHandles, -CandidateWindowHandles, -ExcludeWindowHandles, -DesktopNumbers, -SkipEntryKeys, -KeepPositionedWindows
...
- **Parameters:** -LayoutConfig, -TimeoutSeconds, -PollIntervalSeconds, -FocusWindows, -FocusDelayMs, -MinimumStableDurationSeconds, -CollectiveStabilitySeconds, -ProcessAbsentGraceSeconds, -RequireStableDimensions, -OnWindowStable, -OnDesktopReady
...
| `-OnDesktopReady`               | scriptblock | -       | Callback fired once per virtual desktop as soon as every entry on it is stable or abandoned while another entry is still pending, receiving the desktop number, `@{ LayoutEntry; Window }` per stable entry, and the handles of every window stable in that poll. Never fires in the poll that completes the wait. |
