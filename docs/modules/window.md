...
- **Parameters:** -LayoutConfig, -ConfigPath, -MonitorInfo, -MonitorConfig, -ExistingWindowHandles, -ExpectedWindowState, -DesktopOffset, -SkipExistingWindows, -PinnedHandleMap, -ProtectedWindowHandles, -CandidateWindowHandles, -ExcludeWindowHandles, -KeepPositionedWindows
...
| `-CandidateWindowHandles` | Whitelist: only these handles may be claimed, every other match is dropped from the candidate list before claiming (same point as the two filters above). `Set-WorkspaceWindowLayout` passes the windows the wait phase confirmed stable for ONE desktop when it positions that desktop while the rest still loads, so a window still loading elsewhere - or one another desktop's entry owns - is never claimed here, whatever the title regex says. |
| `-ExcludeWindowHandles`   | Blacklist: these handles are never claimed. `Set-WorkspaceWindowLayout` passes the windows its per-desktop passes already placed when it finishes the remaining desktops. |
...
| `-OnDesktopReady`               | scriptblock | -       | Callback fired once per virtual desktop as soon as every entry on it is stable or abandoned while another entry is still pending, receiving the desktop number and `@{ LayoutEntry; Window }` per stable entry. Never fires in the poll that completes the wait. |
