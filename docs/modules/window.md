...
- **Parameters:** -LayoutConfig, -ConfigPath, -MonitorInfo, -MonitorConfig, -ExistingWindowHandles, -ExpectedWindowState, -DesktopOffset, -SkipExistingWindows, -PinnedHandleMap, -ProtectedWindowHandles, -CandidateWindowHandles, -ExcludeWindowHandles, -DesktopNumbers, -SkipEntryKeys, -KeepPositionedWindows, -AbandonedEntries
...
| `-PinnedHandleMap`       | Optional hashtable from `CurrentLayout.txt` keyed by `"<DesktopNumber>\|<Monitor>\|<Zone>"` → recorded window (`@{ Handle; ProcessId; ProcessName }`). The authoritative source for which duplicate-named window claims each zone: a still-live, process-matching recorded window is reclaimed exactly; without a valid record, candidates already on the entry's virtual desktop (`Get-WindowDesktopIndex`, resolved once per window per run) are preferred, and geometry decides among them. A stale/dead/reused handle is ignored. |
...
| `-CandidateWindowHandles` | Whitelist: only these handles may be claimed, every other match is dropped from the candidate list before claiming (same point as the two filters above). `Set-WorkspaceWindowLayout` passes every window the wait phase has confirmed stable so far when it positions one desktop while the rest still loads, so a window still loading is never claimed, whatever the title regex says. Such a pass searches each entry once and skips the not-found retry ladder: a miss means the window is not stable yet, and the pass after the wait places it. |
| `-ExcludeWindowHandles`   | Blacklist: these handles are never claimed. `Set-WorkspaceWindowLayout` passes the windows its per-desktop passes already placed when it finishes the rest of the layout. |
...
- **Parameters:** -Index, -TimeoutMs (default: 750), -PollIntervalMs (default: 10), -MaxAttempts (default: 3)
...
- **Parameters:** -LayoutConfig, -TimeoutSeconds, -PollIntervalSeconds, -FocusWindows, -FocusDelayMs, -MinimumStableDurationSeconds, -CollectiveStabilitySeconds, -ProcessAbsentGraceSeconds, -RequireStableDimensions, -OnWindowStable, -OnDesktopReady, -PreExistingWindowHandles, -ExcludeWindowHandles
...
| `-PreExistingWindowHandles`     | HashSet[IntPtr] | -   | Windows that existed before the open. One of these counts as stable the first time an entry matches it - nothing to stabilize. |
| `-ExcludeWindowHandles`         | HashSet[IntPtr] | -   | Windows that never match an entry (another workspace's). An entry whose only matches are excluded is abandoned after `ProcessAbsentGraceSeconds`. |
