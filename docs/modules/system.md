...
`-DisplayName` is the label used in log messages **and** the backup subfolder name (`SymbolicLinkMaker` passes the entry's dotted key, e.g. `PowerToys.Settings`); it defaults to `Path`, with the characters a path carries but a folder name cannot hold replaced by `_`. `-BackupRoot` overrides where replaced items are copied and defaults to `<Repo>\Backups\SymbolicLinks`: one subfolder per entry, one timestamped folder per replacement, so every version ever replaced stays side by side with the newest last. A real directory is removed with `-Recurse`; a directory **symlink** deliberately is not, because `-Recurse` there follows the link and deletes the *target's* contents. Creation failures are logged, never thrown.
...
`-DisplayName` is the label used in log messages **and** the backup subfolder name (`SymbolicLinkMaker` passes the entry's dotted key); it defaults to `Path`, with the characters a path carries but a folder name cannot hold replaced by `_`. `-BackupRoot` overrides where replaced items are copied and defaults to `<Repo>\Backups\SymbolicLinks`; it is a **Windows** path, translated into the distribution with `wslpath -a` so the in-distro `cp -a` can write to it. A backup that cannot be translated or copied skips the entry rather than removing a file it could not save. Creation failures are logged, never thrown.
