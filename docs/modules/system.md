...
Before cleanup it runs `Test-RpcServerHealth -Probe` so the preflight verifies this session's live VirtualDesktop COM state rather than only checking that Windows RPC services are running, rehydrating VirtualDesktop cmdlets if preflight recovery unloaded the module. Operations against `Get-DesktopList`, `Get-DesktopFromWindow`, `Get-DesktopIndex`, and `Remove-Desktop` run through the shared RPC retry helpers with exponential backoff; when an operation fails with the RPC-unavailable error family (classified via `Test-RpcUnavailableError`, which also catches wrapped and localized errors), the session's VirtualDesktop COM proxies are reconnected via `Reset-VirtualDesktopState` before the next attempt so stale sessions recover without a fresh shell. In `-EmptyOnly` mode, if an RPC-unavailable error survives the full per-window retry ladder during the occupancy scan, a circuit breaker trips: with occupancy unknowable, removing "empty" desktops could remove occupied ones, so the cleanup aborts and returns `$false` instead of grinding the multi-second retry ladder through every remaining window.
