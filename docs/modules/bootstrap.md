...
10. WSL environment initialization, symbolic links, CoreAiRules enforcement layer (opt-in via `Steps.CoreAiRules`), AI skills links (opt-in via `Steps.AiSkills`), Obsidian CLI flag (opt-in via `Steps.ObsidianCli`, runs [Enable-ObsidianCli](application.md#enable-obsidiancli) with `-CreateIfMissing`), WSL SSH setup
