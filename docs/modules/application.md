...
- **Description:** Opens Windows Terminal in a new window or in the current shell, optionally running one or more commands in separate tabs with custom tab titles. Each command runs in its own tab (base64-encoded internally for reliable execution) and titles persist even after running commands that would normally change the window title. All tabs of one call are chained into a single `wt` invocation (`new-tab ... ; new-tab ...`), which Windows Terminal processes strictly in order - guaranteeing tab order without per-tab process spawns and settle sleeps. Supports administrator privileges and an explicit `-WindowId` for grouping tabs from different calls into the same window.
