...
The PID-stability sampling (4 samples over 750 ms) only runs while the FancyZones process is young (under ~5 s old) - a long-lived process cannot be mid-crash-loop, so a single sample suffices on the happy path. A successful verification is cached for 10 seconds (cleared by `-ForceRestart` and on any failed check), so the several `Start-FancyZones` calls of one workspace open pay for a single readiness pass.
