...
Lists the available VS Code workspace names by enumerating the `*.code-workspace` files in `Projects.Self.VSCodeWorkspaces` (`<repo>\VSCode\Workspaces`) and returning each file's base name.
...
- [`Get-VSCodeWorkspaceNames` in the Application module reference](../../../modules/application.md#get-vscodeworkspacenames) - parameters, usage and behaviour
