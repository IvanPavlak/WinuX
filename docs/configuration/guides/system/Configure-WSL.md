...
Enables the Windows Subsystem for Linux optional feature (if not already enabled) and installs the default WSL distribution read from `DefaultWSLDistribution` in `Configuration.psd1` (if not already installed).
...
| [`DefaultWSLUsername`](../../configuration-reference.md#wsl-configuration) | string | empty string | The Linux account `Configure-WSL` creates non-interactively on first install and makes the distribution default user. The sudo password is always prompted; passwords never live in configuration. |
...
2. Which WSL username should be created and set as the default user?
    - Options: A lowercase Linux username. Leave empty to let the distribution run its own interactive first-launch account setup.
    - Default: Empty - interactive setup.
...
The Linux account `Configure-WSL` creates non-interactively on first install and makes the distribution default user. The sudo password is always prompted; passwords never live in configuration.
...
Reload-PowerShellProfile
$global:Configuration.DefaultWSLDistribution
$global:Configuration.DefaultWSLUsername
...
- [`Configure-WSL` in the System module reference](../../../modules/system.md#configure-wsl) - parameters, usage and behaviour
...
- [`Deploy-CoreAiRules`](Deploy-CoreAiRules.md) - reads the same configuration
