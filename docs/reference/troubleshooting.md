...
Note that [`Invoke-Fastfetch`](../modules/system.md#invoke-fastfetch) measures the
...
`c` ([`Show-TerminalGreeting`](../modules/system.md#show-terminalgreeting), through [`Invoke-Fastfetch`](../modules/system.md#invoke-fastfetch)) resets the font
to the profile default, then presses `Ctrl+Minus` one step at a time - re-reading the window after
each - until the panel fits, `TerminalGreeting.Fastfetch.AutoFit.MaxShrinkSteps` steps (ships 10) have been taken, or
...
2. Make the panel smaller, which is the real fix: shorten the `custom` border rows in your fastfetch
   ...
   logo, so it follows). Raising `TerminalGreeting.Fastfetch.AutoFit.MaxShrinkSteps` in `Configuration.local.psd1` (or
   `-MaxShrinkSteps` for one call) only helps while the terminal can still shrink.

```powershell
Show-TerminalGreeting -MaxShrinkSteps 20
```

   If the font shrinks further than you like, lower the cap instead - `MaxShrinkSteps = 0` resets to
   the default and never shrinks, and `AutoFit.Enabled = $false` skips the fit entirely.
   `Resolve-TerminalGreetingSettings` shows what `c` runs with and warns about a value out of range.
