...
> [!NOTE]
> **Nothing else needs pre-installing.** Bootstrap provisions every tool the engine depends on: the framework WinGet apps (PowerShell 7, Windows Terminal, and PowerToys pinned to 0.100.2), Git via `Install-Git`, and the pinned `VirtualDesktop` 1.5.11 and the other PowerShell modules via `Install-PowerShellModules`. The bundled JetBrainsMono Nerd Font installs once `NerdFonts`/`DefaultNerdFont` is set in `Configuration.local.psd1` (the base ships them empty). Recommended companions (Oh My Posh, fastfetch, VS Code, Firefox) are opt-in via `WinGetApps.local.csv` - see the [Software List](../reference/software-list.md#recommended-companions). The tested dependency versions are tracked in the `TESTED VERSIONS` block in `Windows/PowerShell/Modules/Bootstrap/Data/WinGetApps.csv` (the single source of truth). The `VirtualDesktop` module - which window management relies on - is bootstrap-installed, not a prerequisite (the CI test suite stubs it for the same reason).
