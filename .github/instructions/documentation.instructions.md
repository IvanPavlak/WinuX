...
- **Every exported function has exactly one configuration guide file named after it**, at `configuration/guides/<module>/<Function-Name>.md`. Use the FULL template when the function reads configuration and the STUB template (whose "This function reads no `Configuration.psd1` keys. There is nothing to configure." sentence is verbatim mandatory) when it does not. Both templates are in `AI/Instructions/DocumentationStyle.md`. The 1:1 mapping is checked by `Infrastructure-ConfigurationGuides.Tests.ps1`.
- A `Configuration.psd1` key change updates the guide of every function that reads that key, and a function that starts or stops reading configuration is upgraded or downgraded between the two templates in the same change.
- `configuration/winux-configurator.md` parses `## Configuration Keys` and `## Decisions` out of the full guides, so those headings and the three-sub-bullet shape of a Decisions item (`Options:` / `Default:` / `More detail:`) are a contract, not formatting.
- Guide bodies use RELATIVE links; the sidebar lists the 10 module index pages (`configuration/guides/<module>/README.md`), never the individual function guides.
- After adding/renaming/removing a function: update its entry alphabetically in `modules/<module>.md`, add/rename/remove its configuration guide and its module-index entry, update the module `.psd1` `FunctionsToExport`, and run `List-Functions -ListDiscrepancies` (must report none). CI enforces the same mapping hermetically: `Run-Tests -TestName "Infrastructure"` runs the whole coherence gate (documentation links, manifest completeness in both directions, the function reference pages including alphabetical order, and the configuration guides).
- Fork-only (Custom area) functions use the same entry format in `docs/custom/<module>.md`; their name goes in `Custom.psd1`'s `FunctionsToExport` (the fork-owned manifest, not an engine module's) - that is what makes them autoload.
