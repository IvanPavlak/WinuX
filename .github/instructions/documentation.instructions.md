...
- After adding/renaming/removing a function: update its entry alphabetically in `modules/<module>.md`, add/rename/remove its configuration guide and its module-index entry, update the module `.psd1` `FunctionsToExport`, and run `List-Functions -ListDiscrepancies` (must report none).
